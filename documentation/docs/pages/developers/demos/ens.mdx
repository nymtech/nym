...
and want its traffic to go through the mixnet, this page is the integration
pattern. ENS resolution is only the example: the adapter below is the whole
mechanism, and it applies to any RPC call `ethers` makes, whether that is reading
a balance, resolving a name, or broadcasting a signed transaction.

Two things it does not do. It does not touch signing or key handling, so your
wallet keeps whatever it already uses. And it hides your IP from the RPC provider
rather than hiding what you ask for: the node still sees the calls themselves, so
an account it can already link to you stays linked. See
