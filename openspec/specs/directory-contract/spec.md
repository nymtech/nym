# directory-contract Specification

## Purpose

The directory contract is the on-chain source of truth for nym-node configuration, flipping distribution from per-node HTTP pulls to a push model: each nym-node pushes its ed25519-signed configuration (public keys, addresses, roles, and other categories) to the contract and consumers read it from chain instead of querying every node directly. Node entries are keyed by `(node_id, label)` and stored as opaque bytes, so the contract is schema-agnostic about the payload; writes are authorized by the node's mixnet identity key and ordered by a per-node gap-free sequence for replay protection. A second, admin-managed `curated` entry class holds well-known or override entities that are not bonded nodes. The contract maintains an O(1) incremental LtHash multiset digest over all entries so a whole-directory read can be recomputed and verified against the on-chain digest. Scope is the contract itself; the off-chain retrieval and verification client is a separate capability.
