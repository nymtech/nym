...
## Purpose
Defines the WgPacketTransport data-plane abstraction and the QUIC bridge client used to front the two-hop entry gateway leg in nym-smol-dvpn.
...
The QUIC bridge client SHALL be implemented inline in `nym-smol-dvpn` using `quinn`
...
- **THEN** `nym-smol-dvpn` does not depend on the `nym_bridges` crate
