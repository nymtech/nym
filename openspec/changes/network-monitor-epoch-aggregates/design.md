...
**Consequence.** One extra write per assignment, and `testrun_in_progress` keeps its existing role as the per-node mutex untouched. Rows that are still null long past their lease need clearing on the table's own schedule, or they accumulate. Matching a returning result back to its row needs an identifier rather than `(node, kind, timestamp)`, since the result's timestamp is not the assignment's. The evidence stays at the orchestrator's boundary and does not survive submission, which is correct: the consumer that needs it is the submission policy, and that runs here.
