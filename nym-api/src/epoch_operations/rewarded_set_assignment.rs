...
impl EpochAdvancer {
    fn determine_rewarded_set(
        ...
        // 1. determine exit gateways
        let exit_eligible = all_choices
            .iter()
            .filter(|node| node.0.can_operate_exit_gateway())
            .collect::<Vec<_>>();
        ...
        // 2. determine entry gateways
        let entry_eligible = all_choices
            .iter()
            .filter(|node| {
                node.0.can_operate_entry_gateway() && !exit_gateways.contains(&node.0.node_id)
            })
            .collect::<Vec<_>>();
    ...
    async fn attach_performance(
        ...
        for nym_node in nym_nodes {
            ...
            let performance = self
                ...
                .await
                .performance;
            log::debug!("nym-node {node_id}: stake: {total_stake}, performance: {performance}",);
    ...
    pub(super) async fn update_rewarded_set_and_advance_epoch(
        ...
        match epoch_status.state {
            EpochState::RoleAssignment { next } => {
                ...
                log::info!("attempting to assign the rewarded set for the upcoming epoch...");
                ...
                if let Err(err) = self
                    ...
                    .await
                {
                    log::error!("FAILED to assign the rewarded set... - {err}");
                    Err(err)
                } else {
