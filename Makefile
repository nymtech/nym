...
docker-check-contracts:
	@docker run --rm --platform $(COSMWASM_OPTIMIZER_PLATFORM) \
	  ...
	  --entrypoint /bin/sh \
	  $(COSMWASM_CHECK_IMAGE) -lc 'apt-get update && apt-get install -y --no-install-recommends llvm-dev libclang-dev pkg-config && export PATH="/usr/local/cargo/bin:/usr/local/rustup/bin:$$PATH" && cargo install cosmwasm-check && cosmwasm-check contracts/artifacts/*.wasm'
