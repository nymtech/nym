...
description: "How a web app catches requests that bypass the mixnet, proves a request went through it, and enforces the routing with Content Security Policy."
...
Some traffic never reaches your JavaScript. Record it: your users stay exposed
through this traffic even after the routed part works.
...
[planning an integration](/developers/integrating#decide-fail-open-or-fail-closed-explicitly)
...
Content Security Policy covers the first four. A cross-origin iframe stays out
of reach for CSP too: it fetches under its own policy, not yours.
