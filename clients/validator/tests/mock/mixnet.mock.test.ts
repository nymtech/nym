...
import { ContractStateParams, LayerDistribution, MixnetContractVersion, RewardingStatus, StakeSaturationResponse } from "../../compiledTypes";
...
describe("Mixnet mock tests", () => {
    ...
    it("get Interval Reward Percent", async () => {
        let execute = testHelper.tests(client, "getIntervalRewardPercent", [mixnet], <number>{
        });
    ...
    it("get Rewarding Status", async () => {
        let execute = testHelper.tests(client, "getRewardingStatus", [mixnet, mix_identity, rewardingIntervalNonce], <RewardingStatus>{
            Complete: {},
        });
        expect(execute).toBeTruthy();
    });
    ...
    it("get Circulating Supply", async () => {
        let execute = testHelper.tests(client, "getCirculatingSupply", [mixnet], <string>{
        });
        expect(execute).toBeTruthy();
    });
});