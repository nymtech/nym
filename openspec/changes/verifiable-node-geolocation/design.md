...
The canonical `Location` type lives in the contract's shared types crate, with the `utoipa` schema derive feature-gated as the directory contract does, so contract, service and eventual HTTP consumers depend on one definition rather than converting between two.
