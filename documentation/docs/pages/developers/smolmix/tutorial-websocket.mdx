---
title: "smolmix Tutorial 3: Private WebSocket Echo"
...
# Tutorial 3: Private WebSocket Echo
...
We resolve the hostname via clearnet DNS here for simplicity. For production use, resolve through the mixnet instead (see [Tutorial 2](/developers/smolmix/tutorial-udp) for the DNS pattern).

```rust
    // Resolve via clearnet DNS (see Tutorial 2 for mixnet DNS)
...
Connect to the resolved IP over TCP, then layer TLS on top. This is identical to [Tutorial 1](/developers/smolmix/tutorial#step-5-tcp--tls-through-the-mixnet) — the smolmix `TcpStream` drops in where `tokio::net::TcpStream` would go.
...
<Callout type="info">
For production use, resolve the hostname through the mixnet to avoid leaking DNS queries over clearnet. See [Tutorial 1](/developers/smolmix/tutorial#step-4-resolve-dns-through-the-mixnet) for the DNS-over-mixnet pattern.
</Callout>
