...
- **Trust-anchor abstraction**: a `DirectoryTrustAnchor` seam that yields a trusted digest at a height. The paranoid implementation derives it from the ICS23 proof against an `app_hash` fetched from a configured RPC (phase 1a). Normal mode (a nym-api quorum) and a full tendermint light client are future implementations behind the same seam.
- **Single-node verified read**: an IAVL membership proof for one entry's raw storage key, decoded via the entry value codec.

## Capabilities

### New Capabilities
- `directory-retrieval-client`: a verifiable retrieval client for the directory contract - whole-directory and single-node reads whose integrity is checked against the on-chain LtHash digest (paranoid mode: ICS23 proof against `app_hash`), with node ed25519 signature verification, trust-tier classification, a pluggable trust anchor, and HTTP fallback for unpublished nodes.
...
- Operational: the paranoid path needs an RPC that retained state at the proof height (archival or a recent height). No contract or on-chain changes.
...
- Normal mode (trust a K-of-N nym-api quorum), which additionally needs nym-api to serve a signed digest snapshot (phase 2).
