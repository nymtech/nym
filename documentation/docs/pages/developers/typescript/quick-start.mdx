...
`sdk-full-fat` inlines the WASM and Web Worker as Base64, so no bundler configuration is needed. For smaller bundles, install `@nymproject/sdk` instead and follow the [Bundling guide](/developers/typescript/bundling).
...
- [Bundling](/developers/typescript/bundling) — Webpack and esbuild configurations for the non-full-fat variants.
