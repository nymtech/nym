---
title: "smolmix Tutorial 2: Private DNS & NTP via UDP"
...
# Tutorial 2: Private DNS & NTP via UDP
...
No TLS or HTTP crates needed — this tutorial is pure UDP.
        ...
        match result {
            Ok(Ok((n, _))) => {
                ...
                let ips: Vec<_> = response
                    ...
                    .collect();
                println!("{host:<20} → {} (rtt: {rtt:.1?})", ips.join(", "));
            }
            Ok(Err(e)) => println!("{host:<20} → ERROR: {e}"),
            Err(_) => println!("{host:<20} → TIMEOUT"),
        }
    }
```

Key points                                                                                                                                                
- src/main.rs: replaced 27 lines of hand-rolled format_unix_time + days_to_date with chrono::DateTime::from_timestamp() + .format()                                                                                                          
- tutorial-udp.mdx: same changes in the step-by-step code block, removed the "timestamp helpers" section entirely, and updated the complete code block                                                                                     

✻ Sautéed for 51s                  

❯ the `type BoxError = Box<dyn std::error::Error + Send + Sync>;` is also akward, can't we just use anyhow or something?                                                                                                                       

● Yeah, anyhow::Result is a straight swap. That affects both tutorials though — the echo one uses BoxError too. Want me to do both?                                                                                                            
:
...
async fn main() -> Result<(), BoxError> {
    ...
    let spinner = tokio::spawn(async {
        ...
        let mut i = 0;
        loop {
            eprint!("\rWaiting for NTP response {}", frames[i % frames.len()]);
            i += 1;
    ...
    match result {
        Ok(Ok((n, _))) if n >= 48 => {
            ...
            let dt = chrono::DateTime::from_timestamp(unix_secs as i64, (millis * 1_000_000) as u32)
                .expect("valid timestamp");
            eprintln!("\rNTP response in {rtt:.1?}");
            println!("Unix timestamp: {unix_secs}.{millis:03}");
            println!("UTC: {}", dt.format("%Y-%m-%d %H:%M:%S%.3f UTC"));
        }
        Ok(Ok((n, _))) => eprintln!("\rShort response: {n} bytes (expected 48)"),
        Ok(Err(e)) => eprintln!("\rERROR: {e}"),
        Err(_) => eprintln!("\rTIMEOUT (30s)"),
    }
