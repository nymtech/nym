...
- [x] 2.5 Store entries as ordinary `cw-storage-plus` JSON values rather than a hand-rolled byte codec, and add a round-trip test pinning that `content` bytes survive verbatim through that encoding. Measured on a realistic entry, JSON costs 473 bytes against a compact codec's 304 (598 against 380 with an attestation), which at ten thousand entries is roughly a megabyte of state; that was judged not worth a bespoke format with its own truncation handling and its own migration story
