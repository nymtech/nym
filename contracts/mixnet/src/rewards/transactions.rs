...
pub(crate) fn try_reward_node(
    ...
    if matches!(current_epoch_status.state, EpochState::Rewarding { last_rewarded, .. } if last_rewarded >= node_id)
    {
    ...
    let params = mixnet_params_storage::CONTRACT_STATE
        .load(deps.storage)?
        .params;
    rewarding_info.normalise_profit_margin(params.profit_margin);
    rewarding_info.normalise_operating_cost(params.interval_operating_cost);
...
pub mod tests {
    ...
    mod mixnode_rewarding {
        ...
        trait TestRewarding {
            ...
            fn assert_rewarding(&mut self, node_id: NodeId, performance: Performance) -> Response;
        }
        ...
        impl TestRewarding for TestSetup {
            ...
            fn execute_rewarding(
                ...
                let role = self.node_role(node_id);
                self.execute_fn(
            ...
            fn assert_rewarding(&mut self, node_id: NodeId, performance: Performance) -> Response {
                ...
                self.legacy_rewarding(node_id, performance)
        ...
        mod epoch_state_is_correctly_updated {
            ...
            fn when_target_mixnode_unbonded() {
                ...
                let node_id_unbonded = test.add_rewarded_legacy_mixnode("mix-owner-unbonded", None);
                ...
                test.assert_rewarding(node_id_unbonded, performance);
                assert_eq!(
                ...
                test.assert_rewarding(node_id_unbonded_leftover, performance);
                assert_eq!(
                    EpochState::Rewarding {
                ...
                test.assert_rewarding(node_id_never_existed, performance);
                assert_eq!(
            ...
            fn when_theres_multiple_nodes_to_reward() {
                ...
                let mut ids = Vec::new();
                for i in 0..100 {
                    let node_id = test.add_rewarded_legacy_mixnode(&format!("mix-owner{i}"), None);
                    ids.push(node_id);
                }
        ...
        fn can_only_be_performed_if_in_rewarding_state() {
            ...
            for bad_state in bad_states {
                ...
                let res = test.legacy_rewarding(1, test_helpers::performance(100.0));
        ...
        fn can_only_be_performed_once_epoch_is_over() {
            ...
            let res = test.legacy_rewarding(node_id, performance);
            assert!(matches!(
                res,
        ...
        fn can_only_be_performed_for_nodes_in_rewarded_set() {
            // let mut test = TestSetup::new();
            //
            // let active_node_id = test.add_rewarded_legacy_mixnode("mix-owner-active", None);
            // let standby_node_id = test.add_rewarded_legacy_mixnode("mix-owner-standby", None);
            // let inactive_node_id = test.add_rewarded_legacy_mixnode("mix-owner-inactive", None);
            // let sender = test.rewarding_validator();
            //
            // test.skip_to_next_epoch_end();
            //
            ...
            // let performance = test_helpers::performance(100.);
            // let env = test.env();
            // let res_active = try_reward_node(
            //     test.deps_mut(),
            //     env.clone(),
            //     sender.clone(),
            //     active_node_id,
            //     performance,
            // );
            // let res_standby = try_reward_node(
            //     test.deps_mut(),
            //     env.clone(),
            //     sender.clone(),
            //     standby_node_id,
            ...
            // );
            // let res_inactive =
            //     try_reward_node(test.deps_mut(), env, sender, inactive_node_id, performance);
            //
            // assert!(res_active.is_ok());
            // assert!(res_standby.is_ok());
            // assert!(matches!(
            //     res_inactive,
            //     Err(MixnetContractError::MixnodeNotInRewardedSet { node_id, .. }) if node_id == inactive_node_id
            // ));
        }
        ...
        fn requires_nonzero_performance_score() {
            ...
            // first rewarding
            let res = test.assert_rewarding(node_id, zero_performance);
            ...
            let res = test.legacy_rewarding(node_id, zero_performance);
            assert!(matches!(
                res,
                Err(MixnetContractError::NodeAlreadyRewarded { node_id, .. }) if node_id == node_id
            ));
        ...
        fn correctly_accounts_for_rewards_distributed() {
            let mut test = TestSetup::new();
            let node_id1 = test.add_rewarded_legacy_mixnode("mix-owner1", None);
            let node_id2 = test.add_rewarded_legacy_mixnode("mix-owner2", None);
            ...
            test.add_immediate_delegation("delegator1", Uint128::new(100_000_000), node_id3);
            test.add_immediate_delegation("delegator2", Uint128::new(123_456_000), node_id3);
            test.add_immediate_delegation("delegator3", Uint128::new(9_100_000_000), node_id3);
        ...
        fn correctly_splits_the_rewards() {
            ...
            test.add_immediate_delegation("delegator1", Uint128::new(50_000_000), node_id1);
            test.add_immediate_delegation("delegator1", Uint128::new(200_000_000), node_id2);

            test.add_immediate_delegation("delegator5", Uint128::new(123_000_000), node_id3);
            test.add_immediate_delegation("delegator6", Uint128::new(456_000_000), node_id3);
        ...
        fn emitted_event_attributes_allow_for_delegator_reward_recomputation() {
            ...
            test.add_immediate_delegation("delegator2", Uint128::new(123_456_000), node_id1);

            let del11 = test.delegation(node_id1, "delegator1", &None);
            let del12 = test.delegation(node_id1, "delegator2", &None);
            let del21 = test.delegation(node_id2, "delegator1", &None);
            ...
            for _ in 0..10 {
                ...
                let res1 = test.assert_rewarding(node_id1, performance);
                ...
                let res2 = test.assert_rewarding(node_id2, performance);
            ...
            test.add_immediate_delegation("delegator3", Uint128::new(15_850_000_000), node_id1);
            test.add_immediate_delegation("delegator3", Uint128::new(15_850_000_000), node_id2);

            let del13 = test.delegation(node_id1, "delegator3", &None);
            let del23 = test.delegation(node_id2, "delegator3", &None);
    ...
    mod withdrawing_delegator_reward {
        ...
        fn can_only_be_done_if_delegation_exists() {
            ...
            let node_id1 = test
                .add_rewarded_legacy_mixnode("mix-owner1", Some(Uint128::new(1_000_000_000_000)));
            let node_id2 = test
                .add_rewarded_legacy_mixnode("mix-owner2", Some(Uint128::new(1_000_000_000_000)));
        ...
        fn tokens_are_only_sent_if_reward_is_nonzero() {
            ...
            let low_stake_id =
                test.add_rewarded_legacy_mixnode("mix-owner3", Some(Uint128::new(100_000_000)));
        ...
        fn correctly_determines_earned_share_and_resets_reward_ratio() {
            let mut test = TestSetup::new();
            let node_id_single = test
                .add_rewarded_legacy_mixnode("mix-owner1", Some(Uint128::new(1_000_000_000_000)));
            let node_id_quad = test
                .add_rewarded_legacy_mixnode("mix-owner2", Some(Uint128::new(1_000_000_000_000)));
