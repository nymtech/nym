...
**Not yet on crates.io.** The Stream module is part of the latest SDK source but is not included in the current crates.io release (v1.20.4). Import from Git to use streams — see [Installation](./importing).
</Callout>
...
- [**`StreamId`**](https://docs.rs/nym-sdk/latest/nym_sdk/mixnet/struct.StreamId.html) — 8-byte random identifier (`u64`) generated by the stream opener, used to multiplex streams over a single client.
