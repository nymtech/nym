...
export const BondNymnodeModal = ({
  ...
  denom: CurrencyDenom;
  onBondNymnode: (data: TBondMixNodeArgs, tokenPool: TPoolOption) => void;
  onClose: () => void;
  onError: (e: string) => void;
}) => {
  const [step, setStep] = useState<1 | 2 | 3 | 4>(1);
  const [nymnodeData, setNymnodeData] = useState<MixnodeData>(defaultMixnodeValues);
  ...
  const handleUpdateSignature = async (data: Signature) => {
    ...
    const payload = {
      ...
      mixnode: mixnodeToTauri(nymnodeData),
  ...
  if (fee) {
    return (
      ...
      >
        <ModalListItem label="Node identity key" value={nymnodeData.identityKey} divider />
        <ModalListItem
          label="Amount"
