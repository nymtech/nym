...
1. **Operators Costs (O.C.)**: How many NYM the operator requests to cover their costs per month, [divided by `720`](#nyx-epoch-vs-interval) (this value is set by the operator in the bonding wallet node settings)
...
The O.C. is a value required per month. However, it's paid only in epochs when the node is active. To calculate this, we use a value called `interval` which represents 30 days (approximate month):
