...
(
    gateways_details_id INTEGER NOT NULL,
    -- 'YYYY-MM-DD'
    date                VARCHAR NOT NULL,
    -- 24h uptimes for that day
    ipv4_uptime         INTEGER,
    ...
    FOREIGN KEY (gateways_details_id) REFERENCES gateway_details (id)
);
...
CREATE
INDEX `gateway_ipv4_status_index` ON `gateway_ipv4_status` (`gateways_details_id`, `timestamp` desc);
CREATE
INDEX `gateway_ipv6_status_index` ON `gateway_ipv6_status` (`gateways_details_id`, `timestamp` desc);
