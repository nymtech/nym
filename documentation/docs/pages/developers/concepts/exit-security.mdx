...
| Inside the mixnet (entry + 3 mix layers + exit) | Sphinx (layered) | Each node only knows its previous and next hop |
...
| IPR → remote host | None (Sphinx is mixnet-only) | Remote host sees the IPR's IP, not yours |
...
- **Application-layer encryption closes the gap.** TLS, the Noise Protocol, or any authenticated encryption keeps the payload as ciphertext to the IPR. It still sees destination IP and port, but not content. If you connect with TLS, the IPR sees encrypted ciphertext going to a destination IP: it knows where but not what.
- **Plaintext is plaintext.** Plain HTTP, unencrypted WebSocket (`ws://`), and plain UDP DNS are readable in full at the exit. The exit doesn't know *who* is asking, but it knows *what*.
...
Network-layer anonymity is not application-layer anonymity. The mixnet hides your IP and unlinks sender from receiver, but it does nothing about identifiers you send *through* it:

- **Application identity.** If you log in, send a cookie, or include an API token, the destination knows who you are regardless of the network path. The mixnet anonymises the pipe, not the contents.
- **Fingerprinting.** A stable request pattern, a distinctive TLS or HTTP fingerprint, or a recognisable account defeats network anonymity by correlation. `mix-fetch`'s [default headers](/developers/mix-fetch#default-request-headers) reduce trivial fingerprinting but do not make you indistinguishable from a real browser.
...
- The per-package "Security model" section on [mix-fetch](/developers/mix-fetch#security-model), [mix-dns](/developers/mix-dns#security-model), and [mix-websocket](/developers/mix-websocket#security-model) for the transport-specific exposure tables.
