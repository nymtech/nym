...
- [x] 2.5 The same read tells a node that was measured from one whose assignments never came back: alongside the scores, how many of that node's assignments are still unreturned, with a node that was never assigned absent from the result entirely. No lease deadline is consulted: the submission path already drops a result whose in-flight row has been reaped, so a sample can only be scored while its lease is live
...
- [x] 3.1 Extend the change's migration with the materialised aggregate table keyed `(mixnet_epoch, node_id, test_kind)`, holding the score and the contributing sample count. A row exists only where something was measured, so an absent row is an absent value
- [x] 3.2 Add the corresponding types to `storage/models.rs`
- [x] 3.3 `storage/manager.rs`: idempotent upsert of an aggregate row, a point read for one `(node, epoch)` across kinds, and a read of every aggregate for an epoch
- [x] 3.4 Tests: a repeated upsert neither duplicates nor alters
