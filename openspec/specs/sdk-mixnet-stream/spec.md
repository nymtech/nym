# sdk-mixnet-stream Specification

## Purpose
Stream-layer liveness for SDK mixnet streams: acknowledgement of accepted streams through the dialer's reply path, an opt-in establishment wait, passive liveness introspection, and automatic keepalive with in-band failure on streams whose peers have proven they speak the extension, all wire-compatible with peers that predate it.
## Requirements
...
#### Scenario: Unknown keepalive discriminant is dropped cleanly
- **WHEN** a frame whose msg-type discriminant is not recognised is received
- **THEN** it is dropped as a non-stream message with no effect on any stream
