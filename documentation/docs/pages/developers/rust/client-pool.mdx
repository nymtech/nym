...
<Callout>
Requires the `client-pool` feature, which is on by default. With `default-features = false`, add `features = ["client-pool"]`. See [Installation](/developers/rust/importing#feature-flags).
</Callout>
