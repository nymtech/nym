# Delta: dvpn-quic-bridge — crate renamed to `smoldvpn`

Names only: the requirement text below is unchanged except `nym-smol-dvpn` →
`smoldvpn`. (Noted during this change, out of its scope: this requirement's
"implemented inline / SHALL NOT depend on `nym_bridges`" text appears stale
against the current implementation, which deliberately delegates to the
published `nym-bridges` crate — see `bridge.rs` and the crate `Cargo.toml`.
That divergence should be reconciled by its own change.)
