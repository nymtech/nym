...
- [x] 3.2 Rolling single-claim guard for FRESH grants, implemented at the finalisation seam (`handle_final_credential_claim` -> `grant_or_resume_free_tier`) as a pure `classify_free_tier_claim(record, now, time_cap, claim_window)` over the single `granted_at`: no record -> `FirstClaim` (seed + stamp); `elapsed >= claim_window` -> `FreshReclaim` (seed + re-stamp `granted_at = now`); `time_cap <= elapsed < claim_window` -> `GuardBlocked` -> reject with `FreeTierClaimGuardActive` (v1; task 5 will route this to the garden instead, task 5.8). Windows are `FREE_TIER_TRIAL_TIME_CAP` (10m) + `FREE_TIER_CLAIM_WINDOW` (24h) network-defaults constants. 5 classifier unit tests green
- [ ] 3.3 Seed the volume allowance from the network-defaults constant (reuse the existing byte accounting path)
- [ ] 3.4 Add the session-time clock: time-exhausted when `now - granted_at >= time_cap`, checked at the existing bandwidth-flush cadence (coarse; wall-clock from `granted_at` - disconnecting does not pause it)
- [ ] 3.5 Trigger the exhaustion transition on whichever limit (bytes or time) is reached first
- [x] 3.5a Resume split by whether the peer row survives. (a) EXISTING peer (common: reconnect / restart-reconcile) short-circuits to `CompletedRegistration` in `check_existing_lp_peer` BEFORE finalisation, so the guard never fires and `granted_at` + remaining bytes are untouched - already the behaviour, no change here (task 5.8 refines it into the restricted marker). (b) row GONE + token re-presented (rare: free-tier exhaustion goes to garden not removal, so this is only a transient-error / manual-removal case) reaches `grant_or_resume_free_tier`; while the record is still active (`elapsed < time_cap`) the classifier returns `Resume` -> reseed the allowance for the fresh `client_id` WITHOUT re-stamping `granted_at` (the time cap keeps counting from the original grant), NOT a guard-blocked claim. NOTE: remaining bytes cannot survive a row deletion (fresh `client_id` -> fresh bandwidth row), so a `Resume` reseeds the FULL allowance - acceptable as it is rare and still bounded by the unchanged time cap
...
- [ ] 4.5 Purchase-endpoint full-speed exemption: install a higher-priority `tc` filter matching the purchase-endpoint allowlist -> default unlimited class, ordered ahead of the per-peer pool classifier, so free peers reach checkout at full speed while everything else stays pooled. Both shaping directions + both families; reuse the garden allowlist config (task 5.3) as the single source of truth. Static per free peer - it does NOT toggle on exhaustion; only the non-allowlisted fallthrough changes (pool while on trial, dropped in garden), making trial/garden symmetric on the purchase path. Purchase bytes still count against the byte allowance (kernel-counter metering is not per-destination) - accepted, keeps the allowance a single number
