...
//     storage: &dyn Storage,
//     node_id: NodeId,
// ) -> Result<Option<Box<dyn NodeBond>>, MixnetContractError> {
//     if let Ok(mix_bond) = mixnode_storage::mixnode_bonds().may_load(storage, node_id) {
//         return Ok(mix_bond.map(|b| Box::new(b)));
//     } else {
//         let node_bond = nymnodes_storage::nym_nodes().may_load(storage, node_id)?;
//         Ok(node_bond.map(Box::new))
//     }
// }
