...
The trick is cheap and quick to set up. The core mechanic is a [mislabeled IP range](/operators/residential-ip#the-core-mechanic) owned by a VPS provider, plus us asking operators to set up nodes there or starting our own.
...
Make sure to check [*What to avoid* section](/operators/residential-ip#what-to-avoid) and treat every IP acquired this way as temporary.
...
Cross-check the assigned IP against [ipinfo](https://ipinfo.io), [ipapi.is](https://ipapi.is), [MaxMind](https://www.maxmind.com/en/geoip-demo) and a fraud score ([IPQualityScore](https://www.ipqualityscore.com/) or [Spur](https://spur.us/)) before counting it as residential. See [how to verify classification](/operators/residential-ip#how-to-verify-classification).

###### 5. Setup nodes

If all checks, start a `nym-node` and check the [API status](/operators/residential-ip#nymvpn-api-logic).
...
| [Cogent](https://cogentco.com/en/) | **Carrier ASN**, tier-1 selling colo + transit | Colo and IP transit quoted; the transit market's aggressive discounter | No | **Global, 200+ metros** | **Danger:** heavily-scanned tier-1 whose space is well known to IP-intelligence vendors, so `isp` won't by itself buy a clean reputation. Also known for aggressive peering disputes and de-peering events, meaning possible reachability gaps to specific destinations. Verify per-prefix before scaling | AS174. Registered as a telecom carrier rather than a hosting company, which is why its space often classifies differently from a VPS provider's — but classification is per-prefix, so sample and test before treating any range as residential. **Potentially the most scalable option on the sheet, pending verification.** Part of current Nym network |
...
| [Blythe](https://blythe.org/lander) | Unknown, needs verification | Unknown | Unknown | US | **Notice: Pending Verification.** Public WHOIS and ASN records for this provider are incomplete or unverified via automated tools | Node operators should independently verify prefix ownership and routing details with upstream directly |
...
The IPs existing on Nym network already contain the most useful finding. They split into three methods, and the one that looks most scalable is the **carrier ASN**: Cogent, mfeed and NeocomISP sell colo and IP transit like any host, but their ASN is registered as a telecom, so their space frequently classifies differently from a VPS provider's. Datacenter reliability, static addressing, real bandwidth and a normal commercial relationship — with residential-class classification as a side effect of who the registrant is. Verify per-prefix before betting on it: one org can hold ranges that classify differently.
