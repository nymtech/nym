...
export default function GatewaysTable() {
  ...
  const staleGateways = useMemo(() => (data || []).reduce((acc, g) => {
    const last_updated_utc = g.last_probe ? dayjs(g.last_probe.last_updated_utc) : null;
    ...
    () => [
      ...
      },
      {
        ...
        Cell: ({row}) => {
          return (
            <>
              <Typography ml={2} fontSize="inherit" component="span">
                {(row.original.location as any).city}/{(row.original.location as any).region}
              </Typography>
            </>
          );
        },
      },
      {
        ...
        Cell: ({cell}) => {
          ...
          return (
            <>
              <Typography ml={2} fontSize="inherit" component="span" display="flex" alignItems="center" gap={1}>
      ...
      },
      {
        ...
        Cell: ({cell}) => {
          const value = Math.floor(Number.parseFloat(cell.getValue<string>() || "0") * 100);
          return (
            <>
              <Typography ml={2} fontSize="inherit" component="span" display="flex" alignItems="center" gap={1}>
                <span>{value}%</span>
      ...
      },
      {
        ...
        header: "Uptime",
        Cell: ({cell, row}) => {
          const value: number = ((row.original as any).performance_v2?.uptime_percentage_last_24_hours || 0) * 100;
        ...
        <TableBody>
          {staleGateways.bins.map((r, i) => (
            <TableRow key={`${(i + 1) * staleGatewayBinWidthMinutes}-bin`}>
              <TableCell>{(i + 1) * staleGatewayBinWidthMinutes} mins old</TableCell>
