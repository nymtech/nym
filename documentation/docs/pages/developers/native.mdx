...
<Callout type="info" emoji="ℹ️">
  This approach might be useful if you want to build custom connection logic, but `MixSocket`/`MixStream` will probably be sufficient for the majority of usecases, and is itself built on top of this module. We recommend developers start there with regards to integrations.
</Callout>

- docs TODO LINK
- examples TODO LINK

### [Soon to be deprecated] TcpProxy Module
This module has been essentially superceded by the `MixSocket`. **New features will not be added to it.** The main drawback of this (which is fixed with `MixSocket`/`MixStream`) is that TLS is impossible, as it exposes a localhost port for the consuming process to communicate with.
...
Since traffic is only packaged as Sphinx until it gets to the Exit Gateway, where it is unwrapped into either HTTPS packets (by a Network Requester) or IP packets (by an IP Packet Router), the last hop between the Gateway travels as normal internet traffic. As such, this option has fewer protections than the E2E option against a global passive adversary, but still grants you timing obfuscation and sender-receiver unlinkability between your client software and whatever service it is interacting with.
