...
When we say *Exit Gateway* in connection to Wireguard,, we talk about a `nym-node` running in a mode Exit Gateway (`--mode exit-gateway`) with Wireguard enabled. Such node is listed in [NymVPN application](https://nym.com) as an Exit Gateway and an Entry Gateway for both dVPN (2-hop) and Mixnet (5-hop) options. That's because every Exit Gateway can serve as an Entry, not vice versa for Mixnet and every Gateway with Wireguard enabled option (`--wireguard-enabled true`) can always serve as both Entry and Exit for dVPN (provided that they are correctly [configured](https://nym.com/docs/operators/nodes/nym-node/configuration)).
</Callout>

This chapter explains how is Gateway's performance meassured and what's the design behind selection of Gateways for NymVPN application. Understanding the logic helps operatorts to triage their server and node configuration.

The main source of truth for NymVPN Gateways is an API endpoint [nymvpn.com/api/public/v1/directory/gateways](https://nymvpn.com/api/public/v1/directory/gateways) storing all stats about available nodes. The sections of this chapter explaiun how are these stats measured and what is their significance.

**Please note that even the most perfectly configured Gateway, running on the strongest machine may not always be listed as a suggested node in the app. If you are interested to learn the details why, check out the [Gateway Probe Details page](https://nym.com/docs/operators/nodes/performance-and-testing/gateway-probe-details).**
...
- **Exit Gateway - Mixnet:**
    - Node configured (running with `--mode` argument) as `exit-gateway`, that configuration ensures that:
        - IPR address present
        - NR address present
    - [IPv6 configured](https://nym.com/docs/operators/nodes/nym-node/configuration#quick-ipv6-check)
    - NymTun configured - using [NetworkTunnelManager](https://nym.com/docs/operators/nodes/nym-node/configuration#routing-configuration)
...
- **Entry & Exit Gateway - dVPN (WireGuard mode):**
    ...
    - NymWG configured - using [NetworkTunnelManager](https://nym.com/docs/operators/nodes/nym-node/configuration#routing-configuration)
    - [Metadata port open for WG](https://nym.com/docs/operators/nodes/nym-node/configuration#fixing-metadata-port-showing-not-open-in-probe-results)
    - Wireguard exit policy [configured](https://nym.com/docs/operators/nodes/nym-node/configuration#wireguard-exit-policy-configuration)
    - [IPv6 configured](https://nym.com/docs/operators/nodes/nym-node/configuration#quick-ipv6-check)
    - Recommended: [QUIC bridge setup](https://nym.com/docs/operators/nodes/nym-node/configuration#quic-transport-bridge-deployment)

### How Probe Works

Nym Gateway probe is perfomance measurement program running tests through various routes following the logic described below. Operators can rown their [local instance of Gateway probe](https://nym.com/docs/operators/nodes/performance-and-testing/gateway-probe) or use visit of our dashboards to conveniently see probe results.
...
1. **`mixnet_score`: Mixnet routing performance**
- **High**: > 80% mixnet packet delivery success
- **Medium**: > 60% mixnet packet delivery success
- **Low**: > 10% mixnet packet delivery success
- **Offline**: ≤ 10% mixnet packet delivery success

2. **`score` (Overall): `mixnet routing * WireGuard performance`**
- Formula:
```
...
3. **`load`: Node health indicator (based on ping success)**
- **Low**: > 80% ping success (low load)
- **Medium**: > 40% ping success
- **High**: ≤ 40% ping success (node under stress, high load)
