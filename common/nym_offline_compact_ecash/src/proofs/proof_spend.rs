...
pub struct SpendWitness<'a> {
    // includes skUser, v, t
    #[zeroize(skip)]
    pub(crate) attributes: &'a [&'a Scalar],
...
pub fn generate_witness_replacement(witness: &SpendWitness) -> WitnessReplacement {
    ...
    let r_r_k = grp_params.n_random_scalars(witness.r_k.len());
...
pub fn compute_instance_commitments(
    ...
    let tt_kappa_k = witness_replacement
        .r_lk
        .iter()
        .zip(witness_replacement.r_r_k.iter())
        .map(|(r_l_k, r_r_k)| {
...
impl SpendProof {
    pub fn construct(
        ...
        let responses_r_k = produce_responses(&witness_replacement.r_r_k, &challenge, &witness.r_k);
