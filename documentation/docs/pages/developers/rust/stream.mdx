...
<Callout>
Requires the `stream` feature, which is on by default. With `default-features = false`, add `features = ["stream"]`. See [Installation](/developers/rust/importing#feature-flags).
</Callout>
