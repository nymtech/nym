...
<Callout>
Requires the `socks5` feature, which is on by default. With `default-features = false`, add `features = ["socks5"]`. See [Installation](/developers/rust/importing#feature-flags).
</Callout>
...
- [Exit Gateway Services](/network/infrastructure/exit-services): what an Exit Gateway can observe in proxy mode
- [Exit security](/developers/concepts/exit-security#pin-an-exit): pinning a Network Requester by address or country
