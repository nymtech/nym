---
title: "Demo: routing Ethereum JSON-RPC through the mixnet (ENS)"
description: "How an Ethereum dApp, web3 app or browser wallet routes its JSON-RPC calls through the Nym mixnet: an ethers v6 adapter that swaps the HTTP transport for mix-fetch. Demonstrated by resolving an ENS name and fetching its IPFS site."
...
If you are building an Ethereum dApp, a web3 app, or a browser extension wallet
and want its traffic to go through the mixnet, start with the two questions on the
[developers overview](/developers), because they decide the package before any code
is written.

**Which runtime?** A browser or extension has no raw sockets and no filesystem, so
it runs a WASM client in a Web Worker rather than the native Rust client. That puts
you on the Browser / WebView row.

**End to end, or proxy?** An Ethereum RPC endpoint is a third party that will never
run Nym, so this is proxy mode: your side runs Nym, traffic leaves the mixnet at an
exit gateway, and continues to the RPC provider over the public internet. That is a
weaker position than [end to end](/network/threat-model/configurations/end-to-end),
and it is worth being deliberate about rather than defaulting into. If you have not
picked a configuration yet, [Choose a Defence](/network/threat-model/choose-config)
starts from the adversary instead of the tool.

Those two answers land you on [`mix-fetch`](/developers/mix-fetch), and the adapter
below is how you wire it to `ethers`. ENS resolution is only the example: the
mechanism applies to any RPC call `ethers` makes, whether that is reading a balance,
resolving a name, or broadcasting a signed transaction.

What proxy mode does and does not buy you here. It hides your IP from the RPC
provider; it does not hide what you ask for, since the provider still sees the calls
and an account it can already link to you stays linked. It also does not touch
signing or key handling, so your wallet keeps whatever it already uses. See
[Exit Gateway Services](/network/infrastructure/exit-services) for what the exit
observes, and [baseline hygiene](/developers/swizzle) for the request-shape
discipline that transport alone never provides.

For a wallet connecting to the Nyx chain rather than to Ethereum, that is a
different job: see [Cosmos Kit](/developers/typescript/cosmos-kit).
