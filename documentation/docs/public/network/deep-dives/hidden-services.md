...
url: https://nym.com/docs/network/deep-dives/hidden-services
---
...
2. A client that knows the address can locate the directory entry (the [HSDir](https://spec.torproject.org/rend-spec-v3)) that holds the service's descriptor.
...
One might hope to sidestep SURB generation by letting a set of authorities learn which gateway serves each client. Systems like [Pudding](https://arxiv.org/abs/2311.10825) do exactly this, but for a different problem: contact discovery, not server-location privacy. Knowing the client-to-gateway mapping lets authorities generate the needed SURBs on a client's behalf.
