...
`Accept-Encoding` is forced to `identity` rather than `gzip, deflate, br` because the wasm build has no decompressor. Advertising compression would surface gzip bytes to the caller un-decoded. Responses therefore arrive uncompressed, so large text or JSON bodies transfer more bytes over the slower mixnet path.
