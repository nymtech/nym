...
use dkg::bte::{decrypt_share, keygen, setup, setup_no_fwd_secrecy, Epoch};
...
#[test]
#[ignore] // expensive test
fn full_threshold_secret_resharing_no_epochs() {
    let dummy_seed = [42u8; 32];
    let mut rng = rand_chacha::ChaCha20Rng::from_seed(dummy_seed);
    let params = setup_no_fwd_secrecy();
    ...
    // start off in a default epoch (root);
    ...
    for (reshared_dealing, prior_vk) in resharing_dealings.iter().zip(recovered_partials.iter()) {
        reshared_dealing
            .verify(&params, epoch, threshold, &receivers, Some(*prior_vk))
            .unwrap();
    }
