...
We have been notified that a handful of nodes have been taken down by abuse reports. Here are some useful points on legal support:

- We are working on a response template tailored for Nym operators. For now please use the Tor template [here](https://community.torproject.org/relay/community-resources/eff-tor-legal-faq/tor-dmca-response/) for a quick response, but **modify all "tor" in the text into "proxy server" before you send it** as Nym is not Tor and it's an extra red flag.
