...
**Mixnet** (Go and C/C++) — ephemeral and persistent client creation, sending messages, anonymous replies via SURBs, and listening for incoming messages.
...
**Client Pool and Stream** — no standalone FFI bindings yet. The TcpProxy bindings use the Client Pool internally.
...
- [Go mixnet example](https://github.com/nymtech/nym/blob/develop/sdk/ffi/go/example.go) — full client lifecycle: init, send, receive, SURB reply
