...
The full model (what each hop sees and the trust the exit implies) is on [Exit Gateway Services](/network/infrastructure/exit-services); the comparison with Tor and VPNs is on [Nym vs other systems](/network/threat-model/comparisons). To pin a specific IPR, see [Exit security](/developers/exit-security#pin-an-exit).
