...
use nym_wireguard::peer_controller::{
    GetClientBandwidthControlResponse, PeerControlRequest, QueryVerifierControlResponse,
};
...
impl PeerControllerTransceiver {
    ...
    pub(crate) async fn query_bandwidth(&self, ip: IpAddr) -> Result<i64, Error> {
    ...
    pub(crate) async fn topup_bandwidth(
        ...
        let mut verifier = verifier.ok_or(Error::NoPeer { ip })?;
        ...
        tokio::spawn(async move {
            match request_rx.recv().await.unwrap() {
                PeerControlRequest::GetVerifierByIp {
                    ...
                    response_tx
                        .send(QueryVerifierControlResponse {
                            success: true,
                            verifier: Some(Box::new(MockVerifier { ret: verifier_bw })),
                        })
                        .ok();
                }
                _ => unimplemented!(),
