...
//!   2. **pinned** — our real `nym_smol_dvpn::probe_bridge` (ed25519 cert
...
//!   cargo run --release -p nym-smol-dvpn --example quic-probe

use std::net::SocketAddr;
use std::sync::Arc;
use std::time::Duration;

use nym_smol_dvpn::{probe_bridge, BridgeParams};
...
async fn raw_handshake(addr: SocketAddr, sni: &str, alpn: &[u8]) -> Result<(), BoxError> {
    ...
    println!("    raw:    OK — handshake completed, ALPN={alpn_neg}");
    ep.wait_idle().await;
    Ok(())
}
...
async fn probe_target(name: &str, addr: &str, sni: &str, id_b64: &str) {
    ...
    match probe_bridge(&params, &CancellationToken::new()).await {
        Ok(()) => println!("    pinned: OK — bridge connect + open_bi succeeded"),
        Err(e) => println!("    pinned: FAIL — {e}"),
    }
}
