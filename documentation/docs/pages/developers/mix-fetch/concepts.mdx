...
`mix-fetch` follows the shared [mixnet exit security model](/network/infrastructure/exit-services): the IPR exit sees your destination, and you rely on TLS to keep the payload as ciphertext to it. What that means specifically for HTTP/S:
