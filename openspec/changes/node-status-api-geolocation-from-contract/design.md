...
Selecting `H` is kept separate from reading at it, and the geolocation read takes the height as a parameter. The point of a shared grid is that one height serves every attested contract, so when the directory read lands it should be driven at the same selected height rather than selecting one of its own; a reader that picks its own height makes that two selections to reconcile, which is the thing the grid exists to avoid.
...
The rule is about *replacement*, not about emptiness. Treating every empty result as a failure looks safer and is worse: a network whose geolocator has not written anything yet would then report a fault on every refresh forever, while the thing it refuses to publish is identical to what it is holding. So an empty result is refused only when there is something to lose, and is published with a warning when the held snapshot is empty too.

**Unusable entries are distinguished by reason in logs, not in the response.** `resolve` returns `None` for three different things, and `get_subject` (`verified.rs:251`) tells them apart. All three produce the same served outcome, but `DecodedLocation::UnsupportedVersion` means a payload version has been rolled out ahead of this build and `DecodedLocation::Malformed` is anomalous, since the contract checks payload size but not content. Collapsing all three into "no location" is exactly the failure the migration note warned against.

They are reported in two places, because they are not all observable in one. The refresh sees only what the contract holds, so it reports an unreadable version at warn and a malformed payload at error, per subject. A node with no entry at all is absent from the contract's set entirely and is invisible there; it is reported at the dVPN builder instead, at warn, where the node id is known and where the consequence is - that gateway is about to be dropped from the directory. Warn rather than debug for that one because thin contract coverage is the rollout risk this change carries, and it should be visible without turning debug logging on.
