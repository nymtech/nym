...
pub struct AuthToken2(pub [u8; 32]);
...
mod auth_token_conversion {
    ...
    fn it_is_possible_to_recover_it_from_valid_b58_string() {
        let auth_token = AuthToken2([42; 32]);
        let auth_token_string = auth_token.to_base58_string();
        assert_eq!(
            auth_token,
            AuthToken2::try_from_base58_string(auth_token_string).unwrap()
        )
    }
    ...
    fn it_is_possible_to_recover_it_from_valid_b58_str_ref() {
        ...
        assert_eq!(
            auth_token,
            AuthToken2::try_from_base58_string(auth_token_str_ref).unwrap()
        )
    }
    ...
    fn it_returns_error_on_b58_with_invalid_characters() {
        ...
        assert!(AuthToken2::try_from_base58_string(invalid_chars_token).is_err())
    }
    ...
    fn it_returns_error_on_too_long_b58_string() {
        ...
        assert!(AuthToken2::try_from_base58_string(auth_token_string).is_err())
    }
