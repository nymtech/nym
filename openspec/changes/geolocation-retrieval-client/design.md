...
Among measured entries the default takes the freshest `checked_at`. Majority voting was rejected: it needs three agents to mean anything and there is one. Maximum age is a parameter, because the geolocator's sweep cadence and node-status-api's tolerance are different numbers.
