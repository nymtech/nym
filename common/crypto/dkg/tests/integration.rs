...
//     let combined_ciphertexts = ciphertext
//         .ciphertext_chunks
//         .iter()
//         .map(|share_chunks| share_chunks.iter().sum::<G1Projective>())
//         .collect::<Vec<_>>();
//     let combined_r = hazmat.r().iter().sum::<Scalar>();
//     let combined_rr = ciphertext.r.iter().sum::<G1Projective>();
//
//     // make sure the combined ciphertexts are valid
//     let g1 = G1Projective::generator();
//     for i in 0..remote_public_keys.len() {
//         let expected = remote_public_keys[i].inner() * combined_r + g1 * shares[i].inner();
