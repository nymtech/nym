...
**Choice.** Two independent randomisations. Per issuer, a weighted coin toss on `full_verification_ratio` (default 1.0, so every issuer is audited) decides whether the full audit runs at all; a skipped issuer is still paid, and is recorded as `skipped_verification`. Per audited issuer, the challenge covers `max(min_validate_per_issuer, claimed_issued x sampling_rate)` deposits (defaults 100 and 0.05), drawn uniformly without replacement, or the whole set if the sample would cover it.

**Why.** Deterrence does not require certainty: a signer that cannot predict which deposits will be sampled must be able to answer correctly for all of them, and a permanent ban makes one catch costly. With a small signer set the per-issuer coin toss buys little, so the default audits everyone and the sampling rate is the coverage-versus-cost control. The floor of 100 keeps small cohorts meaningfully checked, where 5% of a small cohort would sample too few.

**Alternative considered.** A per-issuer coin toss below 1.0 to skip whole issuers. Retained as a knob but defaulted off, because the signer set is small enough to audit in full and full coverage is strictly stronger.

**Consequence.** Under the default ratio of 1.0 every signer is audited each day; lowering the ratio leaves a proportional fraction paid on an unverified commitment, and a cheat confined to unsampled deposits goes undetected in either case. A skipped issuer contributes nothing to the deposit union (Decision 11) while still being paid from it. The `dry-run-check-issuer` subcommand forces a single issuer through the audit with the ratio pinned to 1.0, without paying or recording anything.

**Partial sampling now works.** The leaf-count check previously compared `total_leaves()` (the whole-tree size) against the *sample* size, banning any issuer sampled below its full cohort; it now compares against the committed cohort size, and the challenge requests the sample in ascending merkle-index order so a subset merkle proof verifies (see Resolved Questions). Partial sampling is therefore the intended operating mode, and the sampling knobs are the effective control on audit coverage.

### Decision 10: Two-tier misbehaviour response, split at the challenge

**Choice.** Before the challenge, silence is unrewarded but not punished: a signer that does not answer the count query, does not answer the commitment query, or returns a commitment for the wrong expiration date is simply not paid. From the challenge onward, any answer that is not a correct one is treated as cheating and produces a ban with evidence: no response, a bad response signature, an echoed request whose rewarder signature no longer verifies, a self-declared `max_data_response_size` below `MINIMUM_TICKETBOOK_DATA_REQUEST_SIZE` (50), a wrong expiration date, a failing merkle proof, a proof whose leaf count differs from the committed cohort size, a short or incomplete data batch, a deposit-id or expiration mismatch inside a returned ticketbook, a recomputed leaf absent from the proof, or a cryptographically invalid partial signature.
...
**Consequence.** A ban zeroes that day's reward and is persisted with its evidence blob. The two paths that previously fell through the split are now resolved: a commitment for the wrong expiration date is dropped (the issuer claims zero and is neither paid nor banned), and a commitment that lists deposits with no merkle root is a ban. The persistent ban is now effective: the insert writes the operator account into the `operator_account` column that the pre-ban check reads, so a stored ban matches on later days and a caught signer stays excluded until the ban is cleared. The evidence is recorded correctly in its columns.

### Decision 11: The issuance denominator is the rewarder's own observed deposit union

**Choice.** `issued_ratio = claimed_issued / made_deposits`, where `made_deposits` is the set of distinct deposit ids the rewarder has collected from the commitments of issuers that completed the audit without being caught cheating, accumulated over all audited issuers before any reward is computed. The same count, `approximate_deposits`, is what the daily settlement gate is tested against.
...
**Consequence.** The union is completed over all audited issuers before any ratio is computed, so a reward does not depend on the order in which issuers are checked, and the ratio is capped at 1.0, so a claim divided by a union smaller than it makes (for example a coin-toss-skipped issuer) can never exceed a full per-operator slice. A union of size zero yields a ratio of zero for everyone. The `made_deposits` union is accumulated across the audits rather than as each issuer is checked.
...
**Consequence.** Missed epochs need an operator. A failure recorded for an epoch is never retried, because the marker advances regardless, so an epoch recorded as failed is permanently unpaid. The period header is written before any transaction is sent, so a crash between broadcasting and persisting resumes past the period rather than re-paying it, and a header with no details row is reported at startup as possibly unsettled instead of being replayed. A failed settlement's error text is recorded in the `rewarding_error` column, not `rewarding_tx`.

### Decision 13: Persistence is an append-only audit log carrying the full working

**Choice.** Each period writes a header row (budget, whitelist size, per-operator budget, disabled flag), a details row (the epoch-wide measurement, the amount actually spent, the transaction hash or the error, and a `monitor_only` flag) and one row per measured participant carrying the complete working: for block signing the voting power, its share, the signed-block count and the signed ratio; for issuance the claimed count, the share, the sample size, and the skipped and banned flags. Bans additionally write a row holding the reason and the serialised evidence blob. The header row is written before any reward transaction for the period is sent, and a header with no details row (a period that was begun but whose settlement outcome is unknown, since a transaction for it may or may not have been broadcast) is reported at startup as possibly unsettled and never replayed, so a crash between broadcasting and persisting never re-pays a period.
...
**Consequence.** A single failing recipient (for example a malformed address) fails the whole period's payout. Block signing pays the operator account derived from the validator's `nvaloper…` operator address by swapping the bech32 prefix and recomputing the checksum, which assumes the operator and the reward recipient are the same key. Each memo names the period being paid: the block-signing memo names the epoch id and the issuance memo names the cohort's expiration date.

## Risks / Trade-offs

- **The voting-power window is an eligibility cliff.** 100 seconds of downtime at the wrong moment costs a validator the whole epoch, and its share is redistributed to its peers. → Accepted as intended behaviour; documented as a normative requirement so it is a known rule rather than a surprise.
...
- **The audit is a spot-check: it samples `max(100, 5% of the cohort)` per issuer, so a cheat confined to unsampled deposits can go undetected on a given day.** → Accepted trade-off; deterrence rests on unpredictability and a permanent ban rather than full coverage. Every issuer is audited by default (`full_verification_ratio = 1.0`); the per-issuer coin toss only skips issuers if that ratio is lowered.
...
- **`monitor_only` is set for withheld settlements.** A settlement withheld by the daily minimum-deposit gate is flagged with the same `monitor_only` value as a deliberately monitoring-only deployment. → Documented; distinguishing the two is a follow-on change.
...
These were recorded as open questions or current-state gaps when this change was written, and have since been fixed in the `nym-validator-rewarder` crate. The specs above describe the fixed behaviour, and this change is archived to ratify it.

1. **The swapped arguments that made the persistent issuer ban unmatchable (Decision 10).** Fixed: the insert writes the operator account into the `operator_account` column, so a ban is matched by operator account and persists across days. No migration is needed, because the module never ran in production and no swapped rows exist.
2. **The merkle leaf-count check that banned honest issuers under partial sampling (Decision 9).** Fixed: the check compares `total_leaves()` against the number of committed deposits, not the sample size. A second defect found while fixing this - a subset merkle proof only verifies when its leaf indices are sorted - is also fixed: the rewarder requests the sample in ascending merkle-index order (rewarder-only, effective against current signers), and `nym-ticketbooks-merkle::generate_proof` sorts the indices defensively (a shared-crate change that takes effect once every nym-api is redeployed).
3. **The order-dependent, uncapped `issued_ratio` (Decision 11).** Fixed: the deposit union is completed over all audits before any ratio is computed, and the ratio is capped at 1.0.
4. **The two paths that paid an unverified claim (Decision 10).** Fixed: a commitment for the wrong expiration date is dropped (zero claimed, no ban), and a commitment listing deposits with no merkle root is a ban.
5. **The epoch block-count off-by-one and its single-block panic (Decision 2).** Fixed: the block total is counted inclusively (`last_block - first_block + 1`), so a perfect validator scores exactly 1.0 and a single-block epoch has a block total of 1 rather than panicking.
6. **The `--epoch-duration` CLI flag was accepted and silently ignored.** Fixed: `override_config` applies it to the block-signing epoch duration.
7. **Both settlement memos named the wrong period (Decision 14).** Fixed: each memo names the epoch or the expiration date being paid.
8. **A crash between broadcasting and persisting re-paid a period on restart (Decision 12).** Found after this change was written. Fixed: the period header is written before any transaction is sent, so a crash resumes past the period; a header with no details row is reported at startup and never replayed.
9. **One unresolvable validator voided the whole epoch (Decision 3).** Found after this change was written. Fixed: staking details are resolved from the union of the live validator set and the historical valset, and an unresolvable validator is skipped with an error and left unpaid rather than voiding the epoch.
10. **The `--epoch-budget` flag misnamed the daily budget.** Fixed: renamed to `--daily-budget` / `NYM_VALIDATOR_REWARDER_DAILY_BUDGET`, which correctly names the `rewarding.daily_budget` field it sets. Separately, the default audit parameters were retuned for the expected deployment (roughly 15 issuers issuing ~1000-2000 ticketbooks per issuer per day): `full_verification_ratio` 1.0 (audit every issuer every day), `min_validate_per_issuer` 100, `sampling_rate` 0.05 - which the leaf-count and merkle-ordering fixes now make usable, since partial sampling finally works.
...
1. **`ratios.ticketbook_verification` (Decision 5).** It must be part of a sum equal to 1.0 but has no implementation. Keep as a placeholder for a future verification-rewarding module, or remove it from the config surface?
2. **The `_v1` legacy tables and the empty `rewarder/tasks/mod.rs` stub (Decision 13).** Keep the historical tables and the stub module, or clean both up?
3. **`monitor_only` is recorded as true when the daily minimum-deposit gate withholds a settlement (Decision 13).** Distinguish the two cases in the schema, or accept the conflation?
4. **Persistence is N+2 unbatched inserts with no transaction (Decision 13).** A failure partway through leaves a partial period on disk. Wrap a period's writes in a single transaction, or accept it?
5. **The measurement boundary leaks blocks (Decision 2).** A block stored after the epoch tick fires but timestamped before `end_time` is counted in no epoch, and a scraper coverage gap inside an epoch under-scores everyone silently. Tighten the boundary and detect gaps, or accept the under-count?
6. **Issuance has no startup replay.** A day whose run was interrupted is never audited, because the next tick computes a new "yesterday". Add an issuance resync, or accept the skip?
