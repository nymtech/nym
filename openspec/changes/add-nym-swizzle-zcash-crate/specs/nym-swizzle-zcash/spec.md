...
Quantization SHALL accept a range kind distinguishing normal scan ranges from the reorg-verify range. For a sync with a resume point `a`, the crate MUST NOT emit any separate request for the verify range (the `VERIFY_LOOKAHEAD = 10` blocks below `a`); instead it MUST enforce `a - a' >= VERIFY_LOOKAHEAD` on the emitted catch-up range, widening the start by exactly one grid cell (`a' = a' - S`) when the condition fails. The sync driver MUST run the wallet's hash comparison on the verify-window blocks whenever their chunk arrives (chunk order is shuffled and MUST NOT be serialized to obtain them early) and MUST NOT report the sync as committable until the comparison passes.
...
- **WHEN** all chunks of a sync have been fetched but the verify-window hash comparison has not yet been answered
...
### Requirement: Sync execution through swizzled chunks with a transport slot

The crate SHALL define a `BlockSource` trait as the wallet author's slot for their own lightwalletd transport, taking a half-open height range and returning height-tagged, otherwise-opaque blocks (the block type is generic; the crate performs no network I/O). The sync driver SHALL fetch the emitted range via `nym-swizzle` overlapping randomized chunking (shuffled order, seedable with the `[u8; 32]` seed convention for reproducibility) and deliver every block to the wallet together with its classification.
...
- **THEN** every network request is made through their implementation, with request boundaries chosen by the crate (quantized range, swizzled chunks)

#### Scenario: Chunk union covers exactly the emitted range
- **WHEN** a sync executes over an emitted range
- **THEN** the union of requested chunks equals the emitted range — no gaps, no spill past the emitted end — and chunks are issued in shuffled order

#### Scenario: Seeded sync is reproducible
- **WHEN** two syncs run with the same seed, range, and tip
- **THEN** they issue identical chunk requests in identical order
...
The crate SHALL ship a runnable example that connects to a real public lightwalletd (default `zec.rocks`, overridable) to fetch compact blocks through the quantized, swizzled sync path using a `BlockSource` implementation included with the example, and demonstrates the broadcast path end-to-end — schedule, persist the plan to disk, simulate a restart, resume, build at fire time, and deliver to a mock `TxBroadcaster` — without requiring a wallet seed or funds.
