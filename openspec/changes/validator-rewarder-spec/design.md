...
1. **The 20-block voting-power window (Decision 3).** Intended as an eligibility gate, or only ever a weighting sample? An absent validator's stake leaves `total_vp`, so its share is redistributed to its peers rather than left unspent.
2. **`ratios.ticketbook_verification` (Decision 5).** It must be part of a sum equal to 1.0 but has no implementation. Keep as a placeholder for a future verification-rewarding module, or remove it from the config surface?
3. **`--epoch-budget` sets `rewarding.daily_budget` despite its name.** Rename the flag, or leave it?
4. **The `_v1` legacy tables and the empty `rewarder/tasks/mod.rs` stub (Decision 13).** Keep the historical tables and the stub module, or clean both up?
5. **`monitor_only` is recorded as true when the daily minimum-deposit gate withholds a settlement (Decision 13).** Distinguish the two cases in the schema, or accept the conflation?
6. **Persistence is N+2 unbatched inserts with no transaction (Decision 13).** A failure partway through leaves a partial period on disk. Wrap a period's writes in a single transaction, or accept it?
7. **The measurement boundary leaks blocks (Decision 2).** A block stored after the epoch tick fires but timestamped before `end_time` is counted in no epoch, and a scraper coverage gap inside an epoch under-scores everyone silently. Tighten the boundary and detect gaps, or accept the under-count?
8. **Issuance has no startup replay.** A day whose run was interrupted is never audited, because the next tick computes a new "yesterday". Add an issuance resync, or accept the skip?
