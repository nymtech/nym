...
- Fully `tokio`-async. A `CancellationToken` **aborts the setup phase** (deposit +
  signer round-trip is slow) and **tears down a long-lived running tunnel**.
...
```
  common/smol-core            smoltcp stack: channels<IP packet Vec<u8>> → TcpStream / UdpSocket / DNS.
      ...
      └── sdk/rust/smol-dvpn/  crate nym-smol-dvpn
          (nym-smol-dvpn)     + boringtun WG datapath        (1-/2-hop) [NEW]
...
- **`smol-core`** is `smolmix` with the mixnet-specific bridge removed: the
  ...
  DNS resolver. `smolmix` (5-hop) and `nym-smol-dvpn` (2-hop) both sit on it.
...
- **`nym-smol-dvpn`** owns only the WireGuard datapath (boringtun) and the
  transport strategy.
- **Dependency isolation (required):** every *new* dependency `nym-smol-dvpn` needs
  — `boringtun`, `quinn`, `quinn-proto`, `nym_bridges` (git, pinned), and anything
  else not already used elsewhere — is declared **directly in
  `sdk/rust/smol-dvpn/Cargo.toml`**, *not* promoted to the workspace-root
...
> `nym-smol-dvpn` **must be pure Rust: `boringtun` for the WG engine + `smoltcp`
...
- **MTU stepping (actual reference values):** `WG_TUNNEL_OVERHEAD = 80` B/hop, over
  ...
  1280 on mobile**. `nym-smol-dvpn` sizes the smoltcp interface accordingly. **MTU
...
enum GatewayTransport {
    ...
    QuicBridge {
        ...
        host: Option<String>,       // SNI (often a decoy hostname)
...
```
   ...
   CancellationToken:
     • during Provisioning + Registering → ABORT setup (deposit/signer/registration are slow)
...
## 12. Example CLIs (in `sdk/rust/smol-dvpn/examples`)

- **`smol-dvpn-config --gateway <spec>`** — performs an LP registration against a
...
- **`smol-dvpn-topup`** — spends a stored ticket to **top up bandwidth via the
...
`nym-smol-dvpn` reuses their **architecture and wire protocols** but **not their
