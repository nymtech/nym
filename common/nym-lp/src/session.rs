...
impl<'a> std::fmt::Debug for PSQState<'a> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::InitiatorWaiting { .. } => f.debug_struct("PSQState::InitiatorWaiting").finish(),
            Self::ResponderWaiting { .. } => f.debug_struct("PSQState::ResponderWaiting").finish(),
            Self::Completed { .. } => write!(f, "PSQState::Completed {{ session: <opaque> }}"),
        }
    }
}
...
impl<'a> LpSession<'a> {
    ...
    // pub fn outer_aead_key_for_sending(&self) -> Option<Carrier> {
    //     let kkt_state = self.kkt_state.lock();
    //     match &*kkt_state {
    //         &KKTState::Completed { .. } => self.carrier.lock().clone(),
    //         _ => None,
    //     }
    // }
    ...
    pub fn process_kkt_response(&mut self, encrypted_response_bytes: &[u8]) -> Result<(), LpError> {
        ...
        let (initiator_carrier, remote_encapsulation_key) =
                ...
                Ok((response_frame, remote_context)) => {
                    ...
                    match initiator_ingest_response(
                        ...
                        Ok(remote_encapsulation_key) => {
                            let initiator_carrier = Carrier::from_session_secret(
                                session_secret,
                                context.role(),
    ...
    pub fn process_kkt_request(&mut self, request_bytes: &[u8]) -> Result<LpMessage, LpError> {
        ...
        let responder_carrier = Carrier::from_session_secret(
            session_secret,
            context.role(),
            &response_frame.session_id(),
...
mod tests {
    use super::*;
    use crate::{kem_list, peer::mock_peers, replay::ReplayError, sessions_for_tests};
    ...
    */
    ...
    fn test_replay_protection_sequential() {
        for kem in kem_list() {
            let session = sessions_for_tests(kem).1;
    ...
    */
    ...
    fn test_handshake_abort_on_psq_failure() {
        for kem in kem_list() {
            ...
            initiator_session.set_kkt_completed_for_test(&resp.x25519.pk);
