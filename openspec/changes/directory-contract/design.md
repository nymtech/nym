...
- [LtHash implementation] Use an in-house `common/lthash` crate (generic over `digest::ExtendableOutput`, digest 0.11; `blake3` production XOF, `sha3 0.11` dev-only differential-test XOF; concrete `LtHash16 = LtHash<blake3::Hasher>`) rather than the unmaintained `lthash-rs` or the un-scopable `commonware-cryptography`. blake3-via-digest-0.11 was validated to pass `cosmwasm-check`; digest 0.11 already coexists with the workspace's digest 0.10 (a full 0.11 upgrade is upstream-gated by cosmwasm-crypto/tendermint/dalek/arkworks/nym-crypto, and is not needed). Still confirm the build under the pinned rustc 1.86.
