...
| `CHAT_MIN_SCORE` | runtime, optional | our code | Similarity floor below which a chunk is not treated as a source |
...
Retrieval fetches a fixed number of nearest chunks, then discards anything below a similarity floor (`CHAT_MIN_SCORE`). The floor is what makes "this is not covered in the documentation" possible as an answer.

Without it the floor is effectively zero, and since cosine similarity over normalised embeddings is essentially never negative, *every* question returns a full set of chunks no matter how unrelated. The assistant would then be handed ten irrelevant sections for an off-topic question and asked to answer only from them, and the interface would list ten sources beneath a reply saying the topic is not covered.

The trade-off runs both ways: set it too high and genuine questions lose citations they should have had. It is tuned by asking something deliberately outside the corpus and confirming that comes back with nothing, while real questions keep their sources.
