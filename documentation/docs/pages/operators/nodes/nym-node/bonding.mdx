...
We reccomend to topup with 10 NYM per node, as that will suffice your node for a long time, while it's a negligible cost in fiat terms.
...
- Navigate to `nym/scripts/nym-node-setup/auto-bond`
...
In case you also use `autobond.py` you may want to populate other columns as well, here is the explanation ordered to match the CSV: operator-filled first, AUTO-filled last.
...
> Note: Do not keep your `nodes.csv` opened in a UI spreadsheet while running the operation, or the write-back of balances may not be visible / may be
overwritten on save.
