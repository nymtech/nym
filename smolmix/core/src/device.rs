...
impl NymAsyncDevice {
    pub(crate) fn new(
        ...
        // IPR-negotiated MTU (v10) > 1420 fallback for pre-v10 IPRs.
        #[cfg(target_os = "android")]
        const PLATFORM_MTU: Option<usize> = Some(1280);
        #[cfg(not(target_os = "android"))]
        const PLATFORM_MTU: Option<usize> = None;
        const FALLBACK_MTU: usize = 1420;
        capabilities.max_transmission_unit = std::env::var("SMOLMIX_MTU")
            .ok()
            .and_then(|s| s.parse().ok())
            .or(PLATFORM_MTU)
            .or(negotiated_mtu.map(usize::from))
