...
It is also distinct from the [exit gateway services](/network/infrastructure/exit-services) that run on Exit Gateways to support proxy-mode integrations. The Network Requester and IP Packet Router are general-purpose exits, run by Gateway operators, forwarding to arbitrary clearnet destinations. A service provider serves one application, and you are the one running it.
