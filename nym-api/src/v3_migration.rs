...
pub async fn migrate_v3_database(
    ...
    for gateway in all_known {
        ...
        log::debug!("migrating gateway {identity}");
        ...
        if !bonded {
            log::warn!("could not migrate gateway {identity}, as it does not appear to be bonded. all of its data is going to get purged.");
            storage.purge_gateway(gateway.id).await?;
        } else {
            // this is critical issue because it should have never happened
            log::warn!("could not migrate gateway {identity} even though it's still bonded. something bad has happened!");
            bail!("could not migrate gateway {identity}")
        }
    }

    log::debug!("making the column not nullable");
