...
The various components were originally separate binaries. They've been consolidated into a single `nym-node` binary where the role is specified at runtime. This simplifies operation and makes configuration consistent across roles.

In the future, nodes will automatically switch roles based on network conditions. Operators won't need to manually set whether a node is a Gateway or Mix Node—the network will assign roles dynamically each epoch.
...
The current deployment includes over 600 active nodes across approximately 60 countries, operated by independent parties worldwide. Running a node requires meeting minimum hardware specifications, staking NYM tokens as collateral, and maintaining high uptime for rewards.
