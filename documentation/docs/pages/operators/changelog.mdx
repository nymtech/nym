...
**This release comes with breaking changes - please carefully follow the [steps below](#oscypek-special-update)!**
...
This release brings changes which would lead into a *foreign constraint bug* if operators just switched binaries and restarted the node. The fix is to do a little `sqlite` tweak on the node database.
