...
Fix (user-approved scope extension into library code): a bounded orphan buffer inside `StreamMap` — frames for unregistered streams are held (≤64 streams, ≤32 frames each, swept after a 5s TTL by the existing cleanup task) and drained into the stream's reorder buffer on registration. This closes both race windows (Data before `Open` arrives, and Data between `Open` dispatch and `accept()`'s registration) for every stream-module user. Alternatives considered: exposing initial data on `open_stream` (only fixes single-write clients; complementary, not chosen now) and example-level retry (papers over a silent-drop SDK bug). Verified by five new `StreamMap` unit tests (TDD: the race test was written first and failed on the old code) plus repeated mainnet integration runs.
