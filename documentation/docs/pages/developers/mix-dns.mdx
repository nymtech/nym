...
[`@nymproject/mix-dns`](https://www.npmjs.com/package/@nymproject/mix-dns) resolves hostnames to IPs through the Nym mixnet. DNS queries travel as UDP datagrams to a public resolver via the [IPR (Internet Packet Router)](/network/infrastructure/exit-services#ip-packet-router) exit gateway. No TCP setup, no TLS handshake, no `dns.lookup()` to the OS resolver.
...
- [Reference](/developers/mix-dns/guides): configure the resolver.
- [Concepts & security](/developers/mix-dns/concepts): what the resolver sees, and what it doesn't.
