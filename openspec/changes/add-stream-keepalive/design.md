...
4. **Liveness frames refresh `last_activity`.** For armed streams the idle reaper (default 30 min) becomes a backstop and `PeerUnresponsive` (about 3 min) is the effective reaper; the reaper keeps its current meaning for old peers and one-sided deaths. Documented shift.
