...
pub(crate) const LP_DURATION_BUCKETS: &[f64] = &[0.01, 0.05, 0.1, 0.25, 0.5, 1.0, 2.5, 5.0, 10.0];
...
pub(crate) const LP_CONNECTION_DURATION_BUCKETS: &[f64] = &[
    1.0,     // 1 second
    5.0,     // 5 seconds
    10.0,    // 10 seconds
    30.0,    // 30 seconds
...
impl LpConnectionStats {
    ...
    pub(crate) fn record_bytes_sent(&mut self, bytes: usize) {
        self.bytes_sent += bytes as u64;
    }

    /// Emit connection lifecycle metrics for a client connection
    pub(crate) fn emit_lifecycle_client_metrics(&self, graceful: bool) {
    ...
    pub(crate) fn emit_lifecycle_node_metrics(&self, graceful: bool) {
        // Track connection duration
        let duration = self.duration().as_secs_f64();
        add_histogram_obs!(
