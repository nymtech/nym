...
A node must be able to send chain transactions to cover the automated on-chain calls it makes (such as fetching rewards). This depends on `has_sufficient_tokens` and, unlike the three binary conditions above, it does not zero the score - instead it applies a 20% penalty (a `0.8` multiplier) when unmet:
