# nym-swizzle-zcash Specification

> Synced on archive (2026-08-14) from changes `add-nym-swizzle-zcash-crate` and `address-nym-swizzle-pr-review`.

## Purpose

Zcash-specific traffic-shaping built on nym-swizzle: quantized sync-range emission with checkpoint collision, cover-block classification, deterministic sync execution over a transport slot, and decoupled, persistable broadcast scheduling with fire-time transaction building.
