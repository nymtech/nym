...
pub(crate) fn attach_node_details(
    store: &dyn Storage,
...
pub(crate) fn get_node_details_by_id(
    ...
    if let Some(bond_information) = storage::nym_nodes().may_load(store, mix_id)? {
        attach_node_details(store, bond_information).map(Some)
    } else {
        Ok(None)
    }
}

pub(crate) fn get_node_bond_by_identity(
    store: &dyn Storage,
    identity: IdentityKeyRef,
) -> Result<Option<NymNodeBond>, MixnetContractError> {
