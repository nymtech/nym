        ...
        R: CryptoRng + RngCore + Send,
    {
        ...
        while !shutdown.is_shutdown() {
            ...
            if let Some(registration_details) = maybe_auth_res {
                ...
                match AuthenticatedHandler::upgrade(
                    self,
                    registration_details,
                    mix_receiver,
                    is_active_request_receiver,
                ).await {
                    Ok(auth_result) => return Some((auth_result, distributed_trace_id)),
                    Err(err) => {
                        debug!("failed to upgrade to authenticated handler: {err}");
                        return None;
                    }
                }
            }
            ...
            //     if let Ok(auth_result) = AuthenticatedHandler::upgrade(
            //         self,
            //         registration_details,
            //         mix_receiver,
            //         is_active_request_receiver,
            //     )
            //     .await {
            //         return Some(auth_result, distributed_trace_id);
            //     }
            // }
        }
        None
