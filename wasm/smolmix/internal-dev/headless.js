...
const IPR_ADDRESS = params.get('ipr') ||
...
const CONFIG_LABEL = `cover=${ENABLE_COVER ? 'ON' : 'OFF'}, poisson=${ENABLE_POISSON ? 'ON' : 'OFF'}`;
...
async function testStressHttpbin(api) {
  ...
  for (let i = 0; i < STRESS_COUNT; i++) {
    ...
    requests.push({ id: i + 1, url: `https://httpbin.org/bytes/${p.bytes}`, label: p.label });
  }
  ...
  const avgMs = perReq.filter(r => r.ok).reduce((s, r) => s + r.ms, 0) / (okCount || 1);

  log(`  Result: ${okCount}/${STRESS_COUNT} OK, total ${fmtMs(totalMs)}, avg ${fmtMs(avgMs)}/req`);
...
function printSummary(results) {
  ...
  log(`  ${''.padEnd(nameWidth, '-')}${''.padEnd(resultWidth, '-')}${''.padEnd(20, '-')}`);
  ...
  for (const r of results) {
    ...
    log(`  ${r.name.padEnd(nameWidth)}${resultStr.padEnd(resultWidth)}${timeStr}`);
  }
...
async function runSuite() {
  ...
  try {
    ...
    printSummary([{ name: 'Tunnel setup', ok: false, ms: 0, error: String(e) }]);
    return;
  }
