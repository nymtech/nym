# sdk-service-provider-example Specification

> Synced from change `add-echo-service-provider-example` on archive (2026-08-14); implemented in PR #7057.

## Purpose

Runnable SDK examples and documentation demonstrating the service-provider pattern — anonymous request/reply over the mixnet using the stream module and SURBs, with cover traffic and timing obfuscation explicitly configured — plus the stream-router orphan-frame buffering that makes the pattern reliable, and the mainnet integration test that proves it.
