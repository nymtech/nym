...
Nym operator community members have tested a VPS with Netplan and where in some cases `nym-node --mode exit-gateway` was not routing IP packets properly even after running `network-tunnel-manager.sh` script. We are working on a guide to setup Netplan configuration manually for such cases.
