...
- Run in terminal `gulp compile-scss` for a single compilation or gulp watch for continuos compilation of the changes that you make in `*.scss` files. This command should be run in the same folder where `gulpfile.js` and `package.json` are located
