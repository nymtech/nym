...
- After a bounded number of distinct exits (default 3) all fail with no response (timeout) on the current entry gateway, establishment escalates: it re-registers the base client with a different entry gateway and retries exit discovery and rotation on the new entry. Only no-response failures count toward this trigger, matching the spec: a protocol or allocation error fails that one exit but is not common-mode evidence, so it does not consume the entry-rotation budget.
...
- Active-gateway storage transition: `start_nym_client` skips `add_gateway` when `ClientStorage::get_active_gateway_id` already reports an active gateway, and teardown stops only the base-client tasks, not the stored registration. Re-registering with the same `client_id` would therefore reuse the failed gateway. The escalation must clear or replace the stored active gateway before re-registering, then persist the newly selected gateway with the failed one excluded from selection. (Using a fresh `client_id` per entry attempt is the simpler alternative but discards the client identity, so replacing the active gateway in place is preferred.)
