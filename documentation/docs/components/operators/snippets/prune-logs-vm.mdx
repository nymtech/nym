...
**Check `fail2ban` before deleting `/var/log/auth.log`.** If `fail2ban` is configured with a file backend it reads that file, and removing it stops SSH brute-force banning silently - no error, it simply never bans again. Gateways attract constant SSH scanning, so this matters.

```sh
grep -rE '^\s*backend' /etc/fail2ban/jail.local /etc/fail2ban/jail.d/ 2>/dev/null
```

If the backend is not `systemd`, either set `backend = systemd` in `/etc/fail2ban/jail.local` first, or keep `auth.log` and delete only the rest.
</Callout>
...
# only if fail2ban reads the journal (see the warning above):
...
`qcow2` disks default to `discard=ignore` in QEMU. The guest is still told the device supports discard, so `fstrim -av` inside the VM reports gigabytes "trimmed" and exits 0
- while QEMU throws every discard away and the image on the host never shrinks. This is
why operators end up running `virt-sparsify` nightly: the trims were never reaching the host at all.
</Callout>
...
for vm in $(virsh list --all --name | grep -v '^$'); do
  ...
  while [ "$(virsh domstate "$vm")" != "shut off" ]; do
    ...
    if [ "$waited" -ge 180 ]; then
      echo "  $vm did not shut down in 180s - skipping it."
      break
    fi
  done
