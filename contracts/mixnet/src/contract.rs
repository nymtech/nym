...
pub mod tests {
    ...
    fn migration_to_identity_key_based_storage() {
        use crate::migration_helpers::*;
        use mixnet_contract::IdentityStringPublicKeyWrapper;
        use std::ops::Deref;
        ...
        for mix_i in 0..total_mixnodes {
            // make each 1000th node an invalid one to make sure it's correctly captured
            let identity = if mix_i % 1000 == 0 {
                format!("identity{}", mix_i)
            } else {
                format!("identity{}", mix_i)
                    .hash_to_identity()
                    .to_base58_string()
            ...
            let bond = MixNodeBondOld {
                ...
                mix_node: MixNodeOld {
                    ...
                    sphinx_key: hash_to_sphinx_key(format!("sphinx{}", mix_i)).to_base58_string(),
            ...
            mixnodes_owners_old(&mut deps.storage)
                .save(bond.mix_node.identity_key.as_bytes(), &bond.owner)
                .unwrap();
        }
        ...
        for gate_i in 0..total_gateways {
            ...
            let bond = GatewayBondOld {
                ...
                gateway: GatewayOld {
                    ...
                    sphinx_key: hash_to_sphinx_key(format!("sphinx{}", gate_i)).to_base58_string(),
            ...
            gateways_owners_old(&mut deps.storage)
                .save(bond.gateway.identity_key.as_bytes(), &bond.owner)
                .unwrap();
        }
        ...
        for mix_i in 0..total_mixnodes {
            let owner: HumanAddr = format!("mix-owner{}", mix_i).into();
            let identity = if mix_i % 1000 == 0 {
                format!("identity{}", mix_i)
            } else {
                format!("identity{}", mix_i)
            ...
            if mix_i % 1000 == 0 {
                // it shouldn't exist as it was invalid
                assert!(mixnodes_read(&deps.storage)
                    .may_load(identity.as_bytes())
                    .unwrap()
                    .is_none());
                assert!(mixnodes_owners_read(&deps.storage)
                    .may_load(owner.as_bytes())
                    .unwrap()
                    .is_none());
            } else {
                let identity_key = IdentityStringPublicKeyWrapper(
                ...
                let expected_bond = MixNodeBond {
                    ...
                    mix_node: MixNode {
                        ...
                        layer: mix_i % 3 + 1,
                        location: format!("location{}", mix_i),
                        sphinx_key: hash_to_sphinx_key(format!("sphinx{}", mix_i)),
                        identity_key,
                        version: format!("version{}", mix_i),
                    },
                };
        ...
        for gate_i in 0..total_gateways {
            let owner: HumanAddr = format!("gateway-owner{}", gate_i).into();
            let identity_key = format!("identity{}", gate_i).hash_to_identity();
            ...
            let expected_bond = GatewayBond {
                ...
                gateway: Gateway {
                    ...
                    clients_host: format!("clientshost{}", gate_i),
                    location: format!("location{}", gate_i),
                    sphinx_key: hash_to_sphinx_key(format!("sphinx{}", gate_i)),
                    identity_key,
                    version: format!("version{}", gate_i),
                },
            };
