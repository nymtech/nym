...
pub(crate) async fn _gateway_uptime_history(
    ...
    let owner = nym_contract_cache
        ...
        .await
        .ok_or_else(|| ErrorResponse::new("could not determine gateway owner", Status::NotFound))?;
...
pub(crate) async fn _mixnode_uptime_history(
    ...
    let owner = nym_contract_cache
        ...
        .await
        .ok_or_else(|| ErrorResponse::new("could not determine mixnode owner", Status::NotFound))?;
...
pub(crate) async fn _get_mixnode_reward_estimation(
    ...
    let mixnode = status_cache.mixnode_annotated(mix_id).await;
