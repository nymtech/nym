...
The API version (*"semver"*) will be updated 96h (4 days) after the release is labeled as latest, so the operators have time to upgrade their nodes before [getting penalized](/operators/tokenomics/mixnet-rewards#versions-behind-calculation).
</Callout>
