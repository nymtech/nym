...
It’s also possible tu run it within the daemon with the same CLI interface.
...
--gateway <ID>  Run the gateway connectivity test on the given gateway. Skip those tests if not provided
...
```sh
--gateway <ID>    Register to the given gateway
