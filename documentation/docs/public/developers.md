...
export const archProxy = {
  ...
  topology: {
    nodes: [
      ...
      { id: 'x', kind: 'exit', col: 5, label: 'Exit GW', components: ['Network Requester (NR)', 'IP Packet Router (IPR)'] },
...
The dVPN path is native-only: [`nym-smoldvpn`](/developers/smoldvpn) has no WebAssembly build. Its WireGuard datapath (`boringtun`) and userspace IP stack run on the tokio runtime and compile for native targets only, so a browser or WebView cannot load it. In those runtimes, use the mixnet-based mix-* packages instead.
