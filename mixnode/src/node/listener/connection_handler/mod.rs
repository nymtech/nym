...
impl ConnectionHandler {
    ...
    async fn process_received_packet(self: Arc<Self>, sphinx_packet: SphinxPacket) {
        // all processing incl. delay was done, the only thing left is to forward it
        match self
            .packet_processor
            .process_sphinx_packet(sphinx_packet)
            .await
        {
        ...
        while let Some(sphinx_packet) = framed_conn.next().await {
            match sphinx_packet {
                Ok(sphinx_packet) => {
                    ...
                    // synchronously (without delaying inside of course) under higher load
                    // in single and multi-threaded situation
                    let this = Arc::clone(&this);
                    tokio::spawn(this.process_received_packet(sphinx_packet));
                }
                Err(err) => {
                    error!(
