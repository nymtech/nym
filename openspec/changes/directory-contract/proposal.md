...
- New dependencies: an in-house `common/lthash` crate (a generic multiset hash; `blake3` XOF via `digest 0.11`, with `sha3 0.11` as a dev-only test XOF) for the digest, and `prost` (derive-only). Feasibility was confirmed by building a contract that computes a blake3-backed LtHash and passing `cosmwasm-check`. digest 0.11 coexists with the workspace's existing digest 0.10 (no workspace-wide upgrade required).
