...
| `network_summary` | none | Live counts of bonded nym-nodes: total, gateways, mixnodes, entry and exit. These are independent counts reported by the Node Status API, not a breakdown, so they overlap and do not sum to the total. |
