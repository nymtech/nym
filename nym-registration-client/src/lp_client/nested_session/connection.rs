...
impl<'a, S> NestedConnection<'a, S> {
    ...
    async fn receive_raw_packet(&mut self) -> Result<EncryptedLpPacket, LpClientError>
    where
        S: LpTransport + Unpin,
    {
        todo!()
    ...
    S: LpTransport + Unpin,
{
    ...
    async fn send_length_prefixed_packet(
        ...
        self.send_serialised_packet(packet)
            .await
