...
<Callout type="info">
Full API reference: [**@nymproject/mix-websocket**](/developers/mix-websocket/api/globals).
</Callout>
...
See [`SetupMixTunnelOpts`](/developers/mix-tunnel/api/type-aliases/SetupMixTunnelOpts) for the full surface.

## Security model

`mix-websocket` inherits the [smolmix security model](/developers/smolmix#security-model). The browser-specific consequences are:
...
Full reference: [**@nymproject/mix-websocket**](/developers/mix-websocket/api/globals).

| Export | Purpose |
|---|---|
| `MixWebSocket` (class) | WebSocket-like channel over the mixnet. Extends `EventTarget`. |
...
| `MixWebSocketReadyState` (type) | `0 \| 1 \| 2 \| 3` mirroring browser `WebSocket.readyState`. |
