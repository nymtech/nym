...
```sh
IPV4=$(curl -fsS4 https://ifconfig.me)
IPV6=$(curl -fsS6 https://ifconfig.me || true)
PUBLIC_IPS="$IPV4${IPV6:+,$IPV6}"

./nym-node run --id <ID> --mode exit-gateway --public-ips "$PUBLIC_IPS" --hostname "<HOSTNAME>" --location <LOCATION> --accept-operator-terms-and-conditions --wireguard-enabled true
```
