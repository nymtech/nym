...
- **All three OPEN with no error**: your hoster does not filter outbound TCP on these ports, you are good. If users still report issues with your gateway, the cause is IP reputation at the receiving mail provider (see Step 3).
...
Community-run per-exit SMTP egress testing has been reported at [nym-checker](https://github.com/vvmmaann/nym-checker) (a community-maintained project, not an official Nym data source) and can be referenced when diagnosing whether a specific exit is operator-side, hoster-side, or mail-receiver-side. The full investigation that produced these categories and the wording of the unblock template is documented in the forum post [Nym SMTP egress investigation](https://forum.nym.com/t/nym-smtp-egress-investigation/2288).
