...
use wasm_client_core::helpers::{setup_gateway_from_api, FakeWasmDkgClient};
...
use wasm_client_core::{IdentityKey, Recipient};
...
impl MixFetchClientBuilder {
    ...
    async fn start_client_async(mut self) -> Result<MixFetchClient, MixFetchError> {
        ...
        let mut started_client =
            BaseClientBuilder::<FakeWasmDkgClient, _>::new(&self.config.base, storage, None)
                .start_base()
                .await?;
