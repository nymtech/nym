...
**In reality an operator gets more rewards based on [*Operators cost*](#rewards-distribution) and [*Profit margin*](#rewards-distribution) values as well as a value called &alpha;, giving a slight premium to operators with a high self bond. Read chapter [Rewards calculation](#rewards-calculation) to be able to navigate in all the details relevant for operators and delegators**.

<abbr title="In Naive rewarding the node work fraction is same for all nodes in the active set">node_work_fraction</abbr>
