...
pub struct BaseClientBuilder<'a, T> {
...
impl<'a, T> BaseClientBuilder<'a, T> {
    pub fn new(
        base_config: &'a Config<T>,
        key_manager: KeyManager,
        bandwidth_controller: Option<BandwidthController>,
    ...
    pub fn as_mix_recipient(&self) -> Recipient {
        Recipient::new(
            ...
            *self.key_manager.encryption_keypair().public_key(),
            ...
            NodeIdentity::from_base58_string(self.base_config.get_gateway_id()).unwrap(),
        )
    }
    ...
    fn start_cover_traffic_stream(
        ...
        let mut stream = LoopCoverTrafficStream::new(
            ...
            self.base_config.get_average_ack_delay(),
            self.base_config.get_average_packet_delay(),
            self.base_config.get_loop_cover_traffic_average_delay(),
            mix_tx,
            self.as_mix_recipient(),
            topology_accessor,
        );

        if let Some(size) = self.base_config.get_use_extended_packet_size() {
    ...
    fn start_real_traffic_controller(
        ...
        let mut controller_config = real_messages_control::Config::new(
            self.key_manager.ack_key(),
            self.base_config.get_ack_wait_multiplier(),
            self.base_config.get_ack_wait_addition(),
            self.base_config.get_average_ack_delay(),
            self.base_config.get_message_sending_average_delay(),
            self.base_config.get_average_packet_delay(),
            self.base_config
    ...
    async fn start_gateway_client(
        ...
        let mut gateway_client = GatewayClient::new(
            ...
            ack_sender,
            self.base_config.get_gateway_response_timeout(),
            self.bandwidth_controller.take(),
            Some(shutdown),
        );

        gateway_client
            .set_disabled_credentials_mode(self.base_config.get_disabled_credentials_mode());
    ...
    async fn start_topology_refresher(
        ...
        let topology_refresher_config = TopologyRefresherConfig::new(
            self.base_config.get_validator_api_endpoints(),
            self.base_config.get_topology_refresh_rate(),
    ...
    pub async fn start_base(mut self) -> Result<BaseClient, ClientCoreError> {
        ...
        let reply_key_storage = ReplyKeyStorage::load(
            self.base_config.get_reply_encryption_key_store_path(),
        )
        .tap_err(|err| {
        ...
        if !self.base_config.get_disabled_loop_cover_traffic_stream() {
