...
**This release is introducing new variable having an impact on a a node's config score, make sure to read the [chapter below](#node-client-account-balance)!**
</Callout>

#### Node Client Account balance

Every node has its own Nyx account generated with the initial `run` command. This account has it's mnemonic stored in `$HOME/.nym/nym-nodes/<NODE-ID>/data/cosmos_mnemonic`. The balance of every node's client account is accessible on a public endpoint `<NODE_IPv4>:8080/api/v2/auxiliary-details` or `<HOSTNAME>/api/v2/auxiliary-details`. 
...
**Please follow [these steps](/operators/nodes/nym-node/bonding#fund-nym-node-client-nyx-account) to topup your node account.**
