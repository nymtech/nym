...
TODO replace with a flow diagram like others, with client -> websocket connection annotated -> create sphinx packets + send, nym-node(entrygw) unwrap layer + check MAC, forward to NN(mixnode), unwrap + check MAC + hold for $RANDOM_TIME etc etc
```mermaid
---
config:
  theme: neo-dark
...
As each node receives the packet, they are able to decrypt the packet, perform a validity check, and then pass the packet along (in the case of Nym Nodes running as a Mix Node, after a delay).
