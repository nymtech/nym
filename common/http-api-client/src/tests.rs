...
// configured, `matches_current_host` compares the offending (real) hostname
// against `current_url().front_str()`, which is `None` for an unfronted url.
// That comparison can never succeed, so `update_host` bails out early and the
// client gets stuck retrying the same broken, unfronted url forever instead
// of rotating to the next (fronted) entry in the list.
#[test]
#[cfg(feature = "tunneling")]
fn fronting_enabled_stuck_on_unfronted_first_url() {
    ...
    // Since the current url has no front configured, `apply_hosts_to_req` sends
    // the request straight to `nymvpn.com` (unfronted). If that request fails,
    // the "offending" url reported to `maybe_rotate_hosts` is the real host that
    // was actually contacted.
    let offending = Url::parse("https://nymvpn.com/api/").unwrap();
    client.maybe_rotate_hosts(Some(offending));

    // We should have rotated away from the broken, unfronted url to the fronted
    // fallback - but the host-matching bug described above prevents that.
    assert_eq!(
        client.current_url().as_str(),
        "https://nymvpn-frontdoor.global.ssl.fastly.net/api",
