...
**We recommend a more convenient QUICK bridge deployment using a script [`quick_bridge_deployment.sh`](https://github.com/nymtech/nym/blob/develop/scripts/nym-node-setup/quick_bridge_deployment.sh), following the steps below.**

<Callout type="info" emoji="ℹ️">
**QUICK bridge is a requirement for all nodes which enable Wireguard functionality. Note that it this feature is compatible with nodes from `v1.18.0` (platform release [`v2025.17-isabirra`](https://github.com/nymtech/nym/releases/tag/nym-binaries-v2025.17-isabirra)) and newer!**
</ Callout>

<Steps>
###### 1. Download [`quick_bridge_deployment.sh`](https://github.com/nymtech/nym/blob/develop/scripts/nym-node-setup/quick_bridge_deployment.sh) script
- SSH to your server
- Download the script and make executable 
```sh
https://raw.githubusercontent.com/nymtech/nym/refs/heads/develop/scripts/nym-node-setup/quick_bridge_deployment.sh && \
chmod +x quick_bridge_deployment.sh
```
...
```sh
./nym-node-setup/quick_bridge_deployment.sh full_bridge_setup
```
...
- When you are asked to enter forward address, it's your IPv4 used for bonding the node, alongside port `:51822` (an example can be: `172.232.238.161:51822`)
...
Congratulation, you setup QUIC transport bridge! The script offers a standalone tweaks and checks, you can always run it without any argument to see all the options:
```sh
./quick_bridge_deployment.sh
```
<br/>
<AccordionTemplate name="Command output">
root@localhost:~# ./quick_bridge_deployment.sh
iptables-persistent is already installed.
Usage: ./quick_bridge_deployment.sh [command] [options]
