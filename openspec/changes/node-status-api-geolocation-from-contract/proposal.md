...
- The monitor reads the whole geolocation record set once per cycle through `GeolocationClient::verified_geolocation`, backed by a `ProvenTrustAnchor`, and resolves each subject to a single entry with `DefaultResolutionPolicy`.
...
- Both consumers read that one snapshot. The dVPN directory stops reading location out of the persisted `explorer_pretty_bond` JSONB, and the monitor stops writing it there.
...
- `node-status-api-monitoring`: the ordered cycle loses its ipinfo quota check and replaces the per-node geodata sweep with one verified contract read; the geodata caching requirement is replaced wholesale by snapshot semantics; gateway records stop carrying a location in `explorer_pretty_bond`.
...
**Code.** `nym-node-status-api/nym-node-status-api/src/monitor/{mod,geodata}.rs` (the `IpInfoClient`, `Location`, `NodeGeoCache` and `location_cached` machinery), `src/http/{server,state}.rs` (the cache is plumbed through both), `src/http/api/nym_nodes.rs`, `src/cli/mod.rs` and `src/main.rs` (the removed argument).
