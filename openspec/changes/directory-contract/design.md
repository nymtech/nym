...
Both entry classes live in a SINGLE on-chain store under one namespace, keyed by a length-prefixed, namespace-tagged byte key: `[namespace_tag: u8] ++ len_prefixed(id) ++ len_prefixed(label)`, where the node id is `node_id.to_be_bytes()` (numeric ordering) and the curated id is the handle bytes. This is equivalent to a cw-storage-plus composite key `(u8, Vec<u8>, String)`, which supplies the length-prefixing and `KeyDeserialize` for free. A `:`-delimited string key was rejected: it is collision-ambiguous unless `:` is forbidden in every segment (fragile, validation-dependent), and a decimal node id would sort lexicographically rather than numerically.

Entry values are stored as compact RAW BYTES, bypassing cw-storage-plus's JSON/base64 value codec - the `StoredDeposits` pattern (`contracts/ecash/src/deposit.rs`): keep `Path`/`Prefix`/`range_with_prefix`/`KeyDeserialize` for the key, use `storage.set`/`get` with raw bytes for the value. `NodeEntry`/`CuratedEntry` get `to_bytes`/`try_from_bytes` codecs in `nym-directory-contract-common` (the `data` field stays opaque). This removes the per-entry JSON structure (~50 bytes) and base64 inflation (+33% on `data` and the 64-byte signature) - roughly 33-42% of the contract's dominant cost, which matters because this contract is a blob store whose entries live in permanent IAVL state on every validator. The raw stored bytes double as the canonical encoding a client decodes when ICS23-proving an entry (no JSON-canonicalisation concerns). The ~2 KB LtHash accumulator `Item` is likewise stored raw (rewritten on every write). The small structured maps - per-node `sequence`, `allowed_labels`, `config` - stay on cw-storage-plus `Map`/`Item`: negligible overhead, JSON convenient. Trade-off: a hand-rolled, unit-tested value codec per entry type plus a small storage wrapper, against the ~40% permanent-state/gas saving on the blobs.
