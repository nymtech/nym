...
ask() {
  ...
  if grep -qiE "$pattern" <<<"$answer"; then
    ...
    bad "$label — the caveat never appeared"
    printf '        answer: %.300s\n' "${answer//$'\n'/ }"
  fi
}
...
unsuitable() {
  ...
  if ! grep -qiE "$TURNS_AWAY" <<<"$answer"; then
    if grep -qiE "$ABSTAINS" <<<"$answer"; then
      bad "$label — abstained instead of answering (docs have no page on workload fit)"
    else
      bad "$label — never concedes the mixnet is the wrong tool"
    fi
    printf '        answer: %.300s\n' "${answer//$'\n'/ }"
  elif grep -qiE "$NO_FUDGE" <<<"$answer"; then
    bad "$label — offers to weaken the protection to fit the workload"
    printf '        answer: %.300s\n' "${answer//$'\n'/ }"
  else
    ok "$label"
  fi
