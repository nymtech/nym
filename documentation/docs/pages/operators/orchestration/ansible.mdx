...
This automatic flow is slightly harder to setup in the beggining and it's recommended for operators bonding many nodes, as the initial work is worthit by saving the time of bonding a node at a time. 
...
1. Copy `nodes.csvv.example` to your prefered location without `.example` suffix
2. Fill correctly the csv columns for each node you want to bond:
  - `inventory_node_id`: ise node1
  - `hostname`: same like in your `playbooks/inventory/all` (without `https://` !)
  - `ip`: same like `ansible_host`
  - `account`: Nyx account which will bond this node
  - `mnemonic`: Your nyx mnenonic
  - `identity_key`: node identity key - the easieast way to get it is to navigate to `playbooks/` and run: 
  ```sh
...
- [`unbond_all.py`](://github.com/nymtech/nym/tree/develop/scripts/nym-node-setup/auto-bond/unbond_all.py): Unbond all nodes in the csv if provided with mnemonics (`mnemonic` column)
