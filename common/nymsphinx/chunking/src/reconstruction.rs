...
use crate::{ChunkingError, monitoring};
use log::*;
...
mod reconstruction_buffer {
    ...
    fn reconstructing_set_data_works_for_buffers_of_different_sizes() {
        ...
        let raw_fragments: Vec<_> =
            crate::split_into_sets(&mut rand010::rng(), &message, AVAILABLE_PLAINTEXT_SIZE)
                .into_iter()
...
mod message_reconstructor {
    ...
    fn finding_starting_set_id_returns_none_if_message_was_not_fully_received() {
        ...
        let raw_fragments1: Vec<_> =
            crate::split_into_sets(&mut rand010::rng(), &message1, AVAILABLE_PLAINTEXT_SIZE)
                .into_iter()
                .flat_map(|fragment_set| fragment_set.into_iter())
                .map(|x| x.into_bytes())
                .collect();
        ...
        let raw_fragments2: Vec<_> =
            crate::split_into_sets(&mut rand010::rng(), &message2, AVAILABLE_PLAINTEXT_SIZE)
                .into_iter()
                .flat_map(|fragment_set| fragment_set.into_iter())
                .map(|x| x.into_bytes())
                .collect();
