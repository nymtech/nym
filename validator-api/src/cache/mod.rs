...
use mixnet_contract_common::{
    ContractStateParams, Epoch, GatewayBond, IdentityKey, IdentityKeyRef, MixNodeBond,
...
struct ValidatorCacheInner {
    ...
    // this is not an ideal place for it. It's a temporary measure to deal with the merge conflict on hand
    current_epoch: RwLock<Cache<Epoch>>,
}
...
impl ValidatorCache {
    ...
    async fn update_cache(
        ...
        self.inner.current_epoch.write().await.set(current_epoch);
    }
    ...
    pub(crate) async fn current_epoch(&self) -> Cache<Epoch> {
        self.inner.current_epoch.read().await.clone()
    }
    ...
    pub async fn mixnode_details(
        ...
        todo!("this has to be re-evaluated post-merge to make sure it still makes sense");
        ...
        // with HashSets
        let rewarded_mixnodes = &self.inner.rewarded_set.read().await.value;
