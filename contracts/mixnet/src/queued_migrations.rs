...
mod vesting_purge {
    ...
    fn ensure_no_pending_proxy_events(storage: &dyn Storage) -> Result<(), MixnetContractError> {
        let last_executed = crate::interval::storage::LAST_PROCESSED_EPOCH_EVENT.load(storage)?;
        let last_inserted = crate::interval::storage::EPOCH_EVENT_ID_COUNTER.load(storage)?;
        ...
        for maybe_event in crate::interval::storage::PENDING_EPOCH_EVENTS.range(
            storage,
            None,
            None,
            Order::Ascending,
...
pub(crate) use vesting_purge::vesting_purge;
