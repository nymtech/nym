...
//! 4. The agent is folded into the node's [`AuthorisedAgentsView`]
...
/// and updates the runtime view of authorised agents accordingly.
pub(crate) struct NetworkMonitorAgentsModule {
    ...
    /// Shared handle to every structure derived from the authorised-agent set.
    /// Updates are immediately visible to all packet processing threads.
    pub(crate) authorised_agents: AuthorisedAgentsView,
}
    ...
    async fn handle_msg(
        ...
        match exec_msg {
            ExecuteMsg::AuthoriseNetworkMonitor {
                ...
                if let Some(agent) = AuthorisedAgent::parse_announced(
                    mixnet_address,
                    &bs58_x25519_noise,
                    noise_version,
                    bs58_ed25519_identity.as_deref(),
                ) {
                    self.authorised_agents.add_agent(agent).await
                }
            }
            ExecuteMsg::RevokeNetworkMonitor { address } => {
                self.authorised_agents.remove_agent(address).await
            }
            ExecuteMsg::RevokeAllNetworkMonitors => self.authorised_agents.remove_all().await,
