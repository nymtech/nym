...
**CGNAT means no inbound reachability.** That is fine for an *outbound* exit path, but Nym network requires to handle NAT for the gateway's own reachability. The [Swiss setup](/operators/residential-ip/home/swiss) documents in detail what goes wrong when a Nym node sits behind translation, and much of that experience transfers here.
