...
impl<T> DelayQueue<T> {
    ...
    pub fn insert_at(&mut self, value: T, when: Instant) -> Key {
        ...
        if should_set_delay {
            ...
            if let Some(ref mut delay) = &mut self.delay {
                delay.as_mut().reset_at(delay_time);
            } else {
    ...
    pub fn remove(&mut self, key: &Key) -> Expired<T> {
        ...
        if prev_deadline != next_deadline {
            match (next_deadline, &mut self.delay) {
                ...
                (Some(deadline), Some(delay)) => delay.as_mut().reset_at(deadline),
    ...
    pub fn reset_at(&mut self, key: &Key, when: Instant) {
        ...
        if let (Some(ref mut delay), Some(deadline)) = (&mut self.delay, next_deadline) {
            ...
            delay.as_mut().reset_at(deadline);
        }
    }
        ...
        loop {
            if let Some(ref mut delay) = self.delay {
                ...
                let now = crate::wasm_delay_queue::ms(
                    ...
                    crate::wasm_delay_queue::Round::Down,
                );
    ...
    fn normalize_deadline(&self, when: Instant) -> u64 {
        let when = if when < self.start {
            ...
            crate::wasm_delay_queue::ms(when - self.start, crate::wasm_delay_queue::Round::Up)
        };
