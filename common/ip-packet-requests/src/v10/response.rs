...
// v10 differs from v8 only in `ConnectSuccess` (adds `mtu`). bincode is positional
// and rejects trailing bytes, so the tree down to `ConnectSuccess` is redefined
// rather than extended; branches that can't reach it reuse the v8 types.
...
mod tests {
    ...
    // why the field needed a new version rather than extending v8 in place.
    #[test]
