...
mod tests {
    ...
    fn test_full_session_flow() {
        ...
        for kem in kem_list() {
            ...
            session_manager_1
                .init_kkt_for_test(peer_a_sm, &b.x25519.pk)
                .expect("Failed to init KKT for peer A");
            session_manager_2
                .init_kkt_for_test(peer_b_sm, &a.x25519.pk)
            ...
            while rounds < MAX_ROUNDS {
                ...
                if let Some(payload) = i_msg_payload.take() {
                    ...
                    let decoded_packet =
                        parse_lp_packet(&encoded_msg, None).expect("B parse failed");
                ...
                if let Some(payload) = r_msg_payload.take() {
                    ...
                    let decoded_packet =
                        parse_lp_packet(&encoded_msg, None).expect("A parse failed");
                    assert_eq!(decoded_packet.header.counter, counter);
            ...
            let message_a_to_b =
                create_test_packet(1, receiver_index, counter_a, ciphertext_a_to_b);
            ...
            let message_b_to_a =
                create_test_packet(1, receiver_index, counter_b, ciphertext_b_to_a);
    ...
    fn test_bidirectional_communication() {
        ...
        for kem in kem_list() {
            ...
            for i in 0..NUM_MESSAGES {
                ...
                let message_a =
                    create_test_packet(1, receiver_index, current_counter_a, ciphertext_a);
                ...
                let message_b =
                    create_test_packet(1, receiver_index, current_counter_b, ciphertext_b);
            ...
            let (next_recv_a, total_recv_a) =
                session_manager_1.current_packet_cnt(peer_a_sm).unwrap();
            let (next_recv_b, total_recv_b) =
                session_manager_2.current_packet_cnt(peer_b_sm).unwrap();
        ...
        fn test_full_session_flow_with_process_input() {
            ...
            for kem in kem_list() {
                ...
                if let LpAction::HandshakeComplete = action_b3 {
                    println!(
                        "    Responder received third Noise message, produced HandshakeComplete"
                    );
                } else {
                    println!(
                ...
                let plaintext_a_to_b =
                    LpData::new_opaque(b"Hello from A via process_input!".to_vec());
                let plaintext_b_to_a =
                    LpData::new_opaque(b"Hello from B via process_input!".to_vec());
